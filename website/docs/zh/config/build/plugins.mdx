...
`modifyRstestConfig` 的回调参数在运行时是当前 project 已经规范化后的配置对象，但公开 API 不要求你返回内部的 normalized config。为了保持 API 易用性，TypeScript 类型复用面向用户配置的 `RstestConfig`：你可以直接修改传入的 `config` 对象，也可以返回一个符合 `RstestConfig` 结构的局部配置。Rstest 会在回调完成后重新合并、规范化并校验这些改动。该回调也可以是异步函数。
