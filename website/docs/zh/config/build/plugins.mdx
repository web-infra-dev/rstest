...
`modifyRstestConfig` 适合修改 `include`、`exclude`、`testEnvironment`、`resolve`、`source` 等既有 project 配置，但不支持动态添加或删除 Rstest projects、切换 `browser.enabled`、Rsbuild environments 或 Rsbuild plugins。
