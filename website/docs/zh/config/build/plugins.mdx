...
为了兼容已有用法，该回调仍然使用 `RstestConfig` 作为类型，但运行时会校验并拒绝修改影响全局测试计划、project 派生身份或 Rsbuild environment 拓扑的字段。例如修改 `name`、`coverage`、`reporters`、`pool`、`isolate`、`projects`、`browser.enabled`、`plugins`、`extends` 或 `output.distPath` 时，会抛出错误并提示应改为在 Rstest 配置中声明这些选项。
...
`modifyRstestConfig` 适合修改 `include`、`exclude`、`testEnvironment`、`resolve`、`source`、`output.module` 等既有 project 配置，但不支持动态添加或删除 Rstest projects、修改 project `name`、切换 `browser.enabled`、修改 `coverage`、`reporters`、`pool`、`isolate` 等全局选项，或变更 Rsbuild environments 和 Rsbuild plugins。
