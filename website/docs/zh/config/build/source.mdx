...
Rstest 不会用其他 `compilerOptions` 配置代码转换。请通过 [`tools.swc`](/config/build/tools#toolsswc) 配置输出 target，并通过 [`source.decorators`](#sourcedecorators) 显式配置装饰器语法。例如，使用 legacy 装饰器转换和 ES2020 输出的项目可以这样配置：
