...
直接配置 Rstest 时，它不会用其他 `compilerOptions` 配置代码转换。请通过 [`tools.swc`](/config/build/tools#toolsswc) 配置输出 target，并通过 [`source.decorators`](#sourcedecorators) 显式配置装饰器语法。adapter 可能会映射或推断额外的构建选项，请参考所用 [adapter 的文档](/guide/integration/adapters)。例如，直接配置且使用 legacy 装饰器转换和 ES2020 输出的项目可以这样配置：
