...
当 `disableConsoleIntercept` 为 `true` 时，`silent` 仍然生效。此时日志不会经过 reporter 的 console 拦截链路，因此 [onConsoleLog](/config/test/on-console-log) 和 [printConsoleTrace](/config/test/print-console-trace) 仍然不会生效；但在 `silent: 'passed-only'` 下，Rstest 仍会在内部缓存日志，并在失败任务结束后通过原始 console 输出回放对应日志。
:::
