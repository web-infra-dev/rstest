...
External JavaScript ES module、CommonJS module 和 JSON module 会在当前文件的 VM 中执行。CommonJS module 可以 `require()` 其他 CommonJS 或 JSON module，但为了避免依赖落到 worker host realm 中执行，`require()` ES module 会被拒绝；这类依赖请改用动态 `import()`。TypeScript 源码、自定义 loader 等其他扩展名的 external 文件需要先 bundle，再通过 `vmThreads` 运行。
