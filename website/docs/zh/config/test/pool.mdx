...
在 Node 环境中，`fetch()` 返回 VM Promise，但保留原生 `Response`。它的 `json()`、`text()` 等 body 方法仍返回原生 Promise、结果和异常；转换不会延伸到返回的原生对象的方法。`structuredClone()` 失败时也会保留 Node 原生 `DOMException`（`name: 'DataCloneError'`、`code: 25`）。对于这些值，请使用 `await`，并按内容或错误的 `name`／`code` 断言，避免用 VM constructor 做 `instanceof Promise`、`Object` 或 `Error` 检查。
