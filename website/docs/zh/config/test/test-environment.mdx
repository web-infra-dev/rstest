...
`transformMode` 用于控制自定义 environment 是否应该使用类 Node 或类浏览器的默认转换和解析行为。

自定义 environment 默认使用 `transformMode: 'node'`，除非你显式配置。内置 environment 会自动推断该值，所以通常只有配置自定义 environment 时才需要使用这个选项。
...
当 environment 是 DOM 或类浏览器自定义 environment 时，使用 `transformMode: 'browser'`。在这个模式下，如果没有自定义 `resolve.conditionNames`，Rstest 会添加 `browser` condition 来解析面向浏览器的 package entry，并默认 bundle 第三方依赖，从而让 browser field 替换和被打包的资源生效。
...
#### 从 Jest 或 Vitest 迁移自定义 environment \{#migrating-custom-environments}
