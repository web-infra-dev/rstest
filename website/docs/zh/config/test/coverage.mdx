...
- 在 Node 测试或 Chromium Browser Mode 测试中切换到 `coverage.provider: 'v8'`；
- 避免序列化经过 Istanbul 插桩的函数。

在 Chromium Browser Mode 中，`v8` provider 同样不会注入计数器。Firefox 和 WebKit Browser Mode 测试应使用 `coverage.exclude`，或避免序列化经过插桩的函数。
...
`v8` provider 适用于 `node`、`jsdom` 和 `happy-dom` 测试环境，也适用于使用 Playwright provider 的 Chromium Browser Mode。Chromium coverage 从实际执行的浏览器产物收集，并通过 source map 重映射，无需对应用代码插桩。Firefox 和 WebKit 未暴露所需的 coverage API，请对它们使用默认的 `istanbul` provider。
