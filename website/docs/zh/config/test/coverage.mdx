...
`v8` provider 适用于 `node`、`jsdom` 和 `happy-dom` 测试环境。Browser Mode 的稳定支持范围仅限使用 Playwright provider 的 headless、非 watch Chromium 运行。Chromium coverage 从实际执行的浏览器产物收集，并通过 source map 重映射，无需对应用代码插桩。
...
浏览器原生 V8 coverage 目前只分析 page renderer target 中的脚本，不会收集仅在独立 `Worker` 或 `SharedWorker` target 中运行的代码。如果 worker-only 代码也必须计入 coverage，请使用 `istanbul`。
