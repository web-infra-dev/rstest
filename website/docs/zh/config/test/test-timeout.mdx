---
description: 测试的默认超时时间（以毫秒为单位）。设置为 0 禁用超时。
---
...
1. 每个测试用例中的 [timeout](/api/runtime-api/test-api/test#test-1) 配置
2. 测试文件中通过 [`rs.setConfig({ testTimeout })`](/api/runtime-api/rstest/utilities#rssetconfig) 配置的超时时间
3. CLI 中的 `--testTimeout` 选项
4. 配置文件中的 `testTimeout` 选项
5. 默认值 `5000` 毫秒（Browser Mode 下为 `15000` 毫秒）
