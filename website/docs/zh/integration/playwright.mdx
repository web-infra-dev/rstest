...
`definePlaywrightConfig` 会为当前 project 配置默认的 `playwright` fixture。如果使用多 project 配置，请将它添加到每个使用 `@rstest/playwright` 的 Node.js project 的 `extends` 中。配置值必须支持 JSON 序列化，不支持 `launchOptions.logger` 等函数、class instance、client certificate 中直接传入的 `Buffer`，以及依赖当前测试或 retry context 的值。此类配置请改用 `certPath`/`keyPath`/`pfxPath` 或 `test.extend`。如果多个测试文件需要使用另一组选项，可以在共享模块中通过 `test.extend` 覆盖 fixture，并让测试文件统一从该模块导入 `test` 和 `expect`：
...
| Playwright Locator/Page 断言超时 | `5000ms`                                                          |
| Rstest `expect.poll.timeout`     | `definePlaywrightConfig` 的 `expect.timeout`，未设置时为 `5000ms` |
...
这里的 Playwright Locator/Page 断言使用 `10_000ms`，`expect.poll()` 则使用 `2000ms`。两类 API 都可以通过单次调用的 timeout 进一步覆盖默认值。
...
超时时间的优先级依次为：matcher 的 `{ timeout }`、`definePlaywrightConfig` 的 `expect.timeout`、默认值 `5000ms`。`.not` 和 `expect.soft` 同样遵循此规则。外层 test 或 hook 的超时仍可能提前结束断言。

此 helper 还会用 `expect.timeout` 设置 Rstest `expect.poll()` 的默认超时。你可以通过顶层 `expect.poll.timeout` 单独覆盖轮询超时；修改顶层轮询配置不会改变 Playwright Locator/Page 断言的超时。

Rstest 的 [`expect.poll.timeout`](/config/test/expect#expectpolltimeout) 控制 `expect.poll()` 和 Browser Mode 的 `expect.element()`，不影响这里的 Playwright 断言。`page.setDefaultTimeout()` 控制 Playwright 操作的超时，不会改变断言的超时。Rstest 不读取 `playwright.config.ts`。
