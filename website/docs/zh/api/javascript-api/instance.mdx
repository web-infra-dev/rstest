...
当 `unhandledErrors` 不为空时，`status` 为 `'error'`。当本次运行已完成但退出状态非零时，`status` 为 `'failed'`，例如测试或测试文件失败、覆盖率未达阈值、未设置 `passWithNoTests` 时没有找到测试，或者 `globalSetup` 清理失败。watch 模式下，只要会话里还有失败的测试文件，即使当前轮次全部通过，`status` 仍是 `'failed'`。其他情况下，`status` 为 `'passed'`。
...
- **启动失败**——配置阶段抛错，或运行依赖的资源无法启动（dev server、浏览器）。`run()` 和 `watch()` 都会被拒绝。如果 `globalSetup` 在 watch 首轮抛错或编译失败，该轮会先通过 `onResult` 上报 `status: 'error'`，随后 `watch()` 被拒绝。在非 watch 模式下，`globalSetup` 失败仍由 `run()` 返回 `status: 'error'` 的结果。
- **启动之后的致命失败**——bundler 以不会重新挂载文件监听器的方式结束了一次编译。`run()` 返回 `status: 'error'`；在 watch 中，失败的那一轮仍会送达 `onResult`，随后会话关闭；若 `watch()` 尚未 resolve，则会被拒绝。
