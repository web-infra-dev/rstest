...
使用主入口的 [`loadConfig`](/api/javascript-api/rstest-core#loadconfig) 加载配置文件，再将返回值（`{ content, filePath, dependencies }`）直接作为 `config` 传入。
...
`rstest.run()` 会运行一个测试轮次，并返回 `TestRunResult`。只有在运行完全无法开始时（无效的选项或配置，或 node 项目的 dev server 无法启动）Promise 才会被拒绝。此后的一切失败——浏览器项目的 dev server 或浏览器无法启动、`globalSetup` 抛错、编译错误、测试失败——都通过 `status` 报告，不会导致 Promise 被拒绝，具体规则见 [TestRunResult](#testrunresult)。
...
其余字段（`results`、`testResults`、`summary`、`duration`、`snapshotSummary`、`unhandledErrors`、`coverage`、`rerunTestPaths`）的说明见 Reporter API 的 [`onTestRunEnd`](/api/javascript-api/reporter#ontestrunend)。

当 `unhandledErrors` 不为空时，`status` 为 `'error'`。当本次运行已完成但退出状态非零时，`status` 为 `'fail'`，例如测试或测试文件失败、覆盖率未达阈值、未设置 `passWithNoTests` 时没有找到测试，或者 `globalSetup` 清理失败。watch 模式下，只要会话里还有失败的测试文件，即使当前轮次全部通过，`status` 仍是 `'fail'`。其他情况下，`status` 为 `'pass'`。
...
每一轮结束后都会调用 `onResult`，第一轮也不例外。`results`、`summary` 和 `status` 反映整个 watch 会话的状态，`rerunTestPaths` 是这一轮实际执行的文件。

失败分为三类：

- **启动失败**——配置阶段抛错，或 node 项目的 dev server 无法启动。`run()` 和 `watch()` 都会被拒绝。
- **启动之后的致命失败**——浏览器项目的 dev server 或浏览器无法启动、首轮 `globalSetup` 抛错或编译失败，或 bundler 以不会重新挂载文件监听器的方式结束了一次编译。`run()` 返回 `status: 'error'`；在 watch 中，失败的那一轮仍会送达 `onResult`，随后会话关闭；若 `watch()` 尚未 resolve，则以该错误被拒绝——多个错误时合并为 `AggregateError`。
- **可恢复的周期失败**——测试失败、测试文件编译失败，或后续轮次的 `globalSetup` 失败。该轮会上报失败，watch 会话保持打开，等待下一次触发。
...
只有显式传给 `rstest.listTests()` 的 `shard` 才会生效，并将结果限制在选定分片。省略 `shard` 时会列出所有文件；实例配置中的 `shard` 仍会像以前一样被丢弃。
