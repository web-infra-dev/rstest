...
重跑执行出错会通过 `onResult` 以 `status: 'error'` 交付，不会关闭会话；启动失败会使 `watch()` 被拒绝。

失败分为三类：

- **启动失败**——什么都没运行：配置阶段抛错，或运行依赖的资源无法启动（dev server、浏览器等）。`run()` 和 `watch()` 都会被拒绝。
- **启动之后的致命失败**——bundler 以不会重新挂载文件监听器的方式结束了一次编译。`run()` 正常返回 `status: 'error'`；在 watch 中，失败的那一轮仍会送达 `onResult`，随后会话关闭，若 `watch()` 尚未 resolve，则会被拒绝。
- **可重试的失败**——`globalSetup` 抛错或编译失败，或测试文件编译失败。该轮为 `status: 'error'`，`run()` 正常返回，watch 会话保持打开并在下一次触发时重试。
