...
`RunReport` 和 `FailureItem` 自 Rstest 0.10.2 起作为公共类型导出。

- `RunReport`：传递给 `Reporter.onTestRunEnd` 的运行级汇总，包含预先计算的 `status`、`counts`、`failures` 与 `unhandledErrors`。
- `FailureItem`：单个失败用例或文件与其错误的配对。
