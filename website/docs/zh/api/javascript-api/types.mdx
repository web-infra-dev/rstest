...
以下 fixture 生命周期类型从 Rstest 0.11.6 开始作为公开类型导出：

- `FixtureScope`：`'test' | 'file' | 'worker'`。
- `FixtureOptions`：控制 builder fixture `scope` 的选项；只有 test scope fixture 支持 `auto`。
- `FixtureLifecycle`：包含 `onCleanup` 的 builder fixture 生命周期对象。
- `FixtureCleanup`：同步或异步 cleanup callback。
- `Fixtures`：对象写法的 fixture 定义 map。
- `Use`：向对象写法 fixture 暴露值的函数。
- `TestAPIs`：扩展后的 fixture-aware `test` API 类型。

Test scope 对象写法和 scoped builder 写法请参阅 [`test.extend`](/api/runtime-api/test-api/test#testextend)。
