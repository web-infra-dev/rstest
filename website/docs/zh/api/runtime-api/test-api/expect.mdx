...
- **默认 `interval`：** `50`（毫秒）
- **默认 `timeout`：** `1000`（毫秒），Browser Mode 下为 `5000`（毫秒）

你可以通过 [`expect.poll` 配置](/config/test/expect#expectpoll)修改这些默认值。单次调用传入的选项优先级更高。
...
- `expect.poll(...)` 不支持 `resolves`/`rejects`、`toThrow*` 以及快照类 matcher。遇到这类不稳定条件建议使用 `rs.waitFor()`。
...
Mock 匹配器用于验证 `rs.fn` / `rs.spyOn` 的交互行为，包括调用次数、参数、调用顺序、返回值和异步结果。
