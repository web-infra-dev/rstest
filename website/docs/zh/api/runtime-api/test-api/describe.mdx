...
`TestOptions` 支持的字段与 [`test`](/api/runtime-api/test-api/test#testoptions) 上一致：`timeout`、`retry`、`repeats`、`meta`（`meta` 为 0.11.1 新增）。这些选项会作为**可继承的默认值**向子套件和测试传播：

- 测试级别的选项始终优先于套件级别的值。
- 嵌套的 `describe` 会继承父级选项，最终以最近一层的值为准。
- 未单独设置 `timeout` 的测试，回退到最近一层套件的 `timeout`，再回退到 [`test.testTimeout`](/config/test/test-timeout)。
- 元数据会从父套件向子套件或测试合并；子级顶层同名字段会覆盖父级字段，继承值会为每个子级复制。
