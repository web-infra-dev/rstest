---
title: Hooks
description: Hooks 允许你在测试或测试套件执行之前或之后运行初始化和清理逻辑。
---
...
`ctx.meta` 为 0.11.1 新增，是当前 suite 的可变元数据对象。如果 suite 通过 `describe(name, { meta }, fn)` 声明，`ctx.meta` 会以应用完套件继承后的元数据作为初始值。文件级 hooks 写入的元数据会出现在 `TestFileResult.meta`；`describe` 内 hooks 写入的元数据会出现在传给自定义 Reporter 的对应 suite result 上。
