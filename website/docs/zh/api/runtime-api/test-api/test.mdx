...
### Object form fixture 的生命周期

Object form fixture 函数接收两个参数：
...
### Named builder fixture

<ApiMeta addedVersion="0.11.6" />

Named builder form 直接返回 fixture 值，不需要调用 `use`。它的第二个参数提供 `onCleanup`，用于注册一个在测试及其 per-test hooks 结束后运行的 callback。Builder 和 cleanup callback 都可以是异步函数。
...
Named builder 属于 test scope：Rstest 会为每次 test attempt 创建新值，并在开始下一次 attempt 前执行 cleanup。需要请求其他 fixture 或 `TestContext` 字段时，请直接在第一个参数中使用对象解构。
