...
省略 options 参数时会创建 test scope builder fixture，例如 `.extend('client', fixture)`。对象写法仍可用于通过 `await use(value)` 生命周期定义 test scope fixture。Setup 默认按需执行；设置 `auto: true` 后自动执行。自动 file 或 worker fixture 在各自的 scope 边界内仍然只执行一次。
