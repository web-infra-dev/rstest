...
- **类型：** `test.extend(fixtures: Fixtures) | test.extend(name, fixture) | test.extend(name, options, fixture)`
...
对象写法中的 test scope fixture 函数接收两个参数：
...
Test scope fixture 函数的第一个参数中同时包含 `TestContext`，因此你可以在 fixture 内直接读取当前测试信息或使用 `expect`：
...
使用 `scope` 选项控制 fixture 实例的存活时间：

| Scope      | Setup 和 cleanup 边界                                                                                                               | 可用依赖                                      |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| `'test'`   | 每次 test attempt 创建一次，包括每次 retry；这是默认值。                                                                            | Test、file、worker fixture 和 `TestContext`。 |
| `'file'`   | 每个测试文件创建一次，在该文件的所有测试和 suite hook 完成后清理。                                                                  | File 和 worker fixture。                      |
| `'worker'` | 每个 worker 创建一次，在 worker 结束时清理。设置 `isolate: false` 后可以跨文件共享；watch rebuild 会先替换 worker，再运行新的构建。 | 仅 worker fixture。                           |

File 和 worker scope fixture 无法访问 `TestContext`，因为在它们的完整生命周期内不存在唯一的当前测试。Fixture 只能依赖生命周期不短于自身的 fixture。比如 worker fixture 依赖 file 或 test fixture 时，Rstest 会报告错误。

File 和 worker scope 通过 builder 写法定义，该写法依次接收 `name`、`options` 和 fixture。Fixture 函数直接返回值，并可以通过 `onCleanup` 注册一个 cleanup：
...
File 和 worker scope fixture 必须在测试文件顶层定义。它们仅在测试、hook 或其他 fixture 请求时按需初始化，不支持 `auto`，也不能覆盖已存在的 fixture。这些限制确保其依赖图在整个 file 或 worker 生命周期内保持稳定。

省略 options 参数时会创建 test scope builder fixture，例如 `.extend('client', fixture)`。对象写法也始终定义 test scope fixture。它的元组选项只识别 `auto`，不识别 `scope`；`[value, { scope: 'file' }]` 这样的数组仍是普通的 test scope fixture 值。
...
Fixture 默认是按需执行的：只有测试或 per-test hook callback 中解构了它（或被其他 fixture 依赖）才会运行。如果你希望某个 test scope fixture 对每个测试都自动生效——即使没有 callback 解构它——可以使用元组语法并设置 `{ auto: true }`：
