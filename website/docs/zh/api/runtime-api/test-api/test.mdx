...
对象写法通过元组选项继续使用 `await use(value)` 生命周期：
...
Builder 写法依次接收 `name`、`options` 和 fixture。Fixture 函数直接返回值，并可以通过 `onCleanup` 注册一个 cleanup：
...
省略 options 参数时会创建 test scope builder fixture，例如 `.extend('client', fixture)`。Setup 默认按需执行；设置 `auto: true` 后自动执行。自动 file 或 worker fixture 在各自的 scope 边界内仍然只执行一次。
