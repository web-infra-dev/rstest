...
| `'worker'` | 每个 worker 创建一次，在 worker 结束时清理。设置 `isolate: false` 后可以跨文件共享。 | 仅 worker fixture。                           |
...
File 和 worker scope fixture 使用 builder 写法。它依次接收 `name`、`options` 和 fixture。Fixture 函数直接返回值，并可以通过 `onCleanup` 注册一个 cleanup：
...
省略 options 参数时会创建 test scope builder fixture，例如 `.extend('client', fixture)`。对象写法仍可用于通过 `await use(value)` 生命周期定义 test scope fixture。覆盖 file 或 worker scope fixture 时，必须再次指定相同的 `scope`；省略或更改 scope 都会报错。

Setup 默认按需执行；设置 `auto: true` 后自动执行。自动 file 和 worker fixture 会在首个相关文件的 suite hook 之前初始化，并且在各自的 scope 边界内仍然只执行一次。
