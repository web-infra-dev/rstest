...
- `meta?: Record<string, unknown>` — 测试结果的初始元数据。如果测试位于带有 `meta` 的 `describe` 中，会继承套件元数据，测试级别的同名字段会覆盖继承值。
...
你可以使用 `context.task.meta` 给当前测试结果附加可 JSON 序列化的元数据。自定义 Reporter 和 programmatic API 可以从 `TestResult.meta` 读取这些数据：
