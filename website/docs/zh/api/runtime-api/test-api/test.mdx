...
- **类型：** `test.extend(fixtures: Fixtures) | test.extend(name, fixture)`
...
具名 fixture 写法会直接返回 fixture 值，不需要调用 `use`。它的第二个参数提供 `onCleanup`，用于注册一个在测试及其 per-test hooks 结束后运行的 callback。fixture 函数和 cleanup callback 都可以是异步函数。
...
具名 fixture 函数属于 test scope：Rstest 会为每次 test attempt 执行这些函数，并在开始下一次 attempt 前执行 cleanup。普通值会在不同 attempt 之间复用；如果可变状态需要在每次 attempt 之间隔离，请使用 fixture 函数。

Fixture 名称必须是静态可知的 ASCII JavaScript identifier，TypeScript 才能准确暴露一个新的 context 字段。此 overload 不接受类型为 `string` 的值、`` `slot${string}` `` 这类 template literal pattern、`base-url` 这类需要带引号解构的名称，以及 `TestContext` 的内置字段。它支持与 Function 属性重名的名称，例如 `name` 和 `length`。
