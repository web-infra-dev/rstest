...
如果请求已经通过 `rs.mock` 或 `rs.doMock` 注册，`rs.importMock` 会优先使用已注册的 factory。没有注册 factory 时，匹配的 `__mocks__` 手动 mock 仍然会生效，之后才回退到普通的自动 mock。请求也可以来自变量，但调用方需要位于 Rstest 打包的测试模块中。

如果运行时请求无法解析到 bundle 中的模块或已注册的 mock，`rs.importMock` 会 reject，并报告模块不存在。
...
请求保存在变量中时，只要调用方位于 Rstest 打包的测试模块中，Rstest 也会将调用改写为同步 runtime helper。目标模块仍需要存在于 bundle 或 mock 注册表中。
