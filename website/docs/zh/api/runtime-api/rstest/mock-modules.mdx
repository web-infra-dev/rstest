---
title: Mock modules
description: Rstest 支持对模块进行 mock，这使得你可以在测试中替换模块的实现。
---
...
异步获取模块的 mock 导出。如果需要同步加载 CommonJS 模块的 mock，请使用 [`rs.requireMock`](#rsrequiremock)。

返回结果取决于模块的 mock 设置：

- 已通过 `rs.mock` 或 `rs.doMock` 注册 mock 时，返回已注册的 mock，包括 factory 返回的导出、手动 mock 或自动 mock 的模块。
- 未注册 mock，且参数是字符串字面量时，加载匹配的 `__mocks__` 文件（如果存在）。这种用法不需要先调用 `rs.mock`。
- 两者都不存在时，返回的 Promise 会被拒绝，错误信息会指出找不到的模块。

已注册的 factory 优先于匹配的 `__mocks__` 文件。factory 中的函数会按原样返回，不会自动转换成 `rs.fn()`。如果需要自动 mock，请先通过 `rs.mock('./sum', { mock: true })` 注册，再加载模块。
...
在 Node 和 Browser 模式下，传入变量路径时，`rs.importMock` 会尝试查找已注册的 mock。可以先通过 `rs.mock` 或 `rs.doMock` 以字面量路径注册目标模块的 mock，再通过变量路径获取它。相对路径以调用 `rs.importMock` 的文件为基准解析。
...
`rs.mock` 会被提升；`rs.doMock` 则在执行时生效，因此需要在 `rs.importMock` 之前调用。变量路径只用于查找已注册的 mock，不会在运行时查找新的模块或 `__mocks__` 文件，即使这些文件已经存在于磁盘上。

## rs.requireMock

- **类型：** `<T = Record<string, unknown>>(path: string) => T`

同步获取 CommonJS 模块的 mock 导出。通过 `rs.mockRequire` 或 `rs.doMockRequire` 注册 mock，可以按 CommonJS 的规则解析目标模块。如果需要异步加载，请使用 [`rs.importMock`](#rsimportmock)。

与 `rs.importMock` 一样，已注册的 mock 优先于匹配的 `__mocks__` 文件。字符串字面量可以直接加载手动 mock，无须提前注册。

传入变量路径时，`rs.requireMock` 会尝试查找已注册的 mock，不会在运行时加载新的模块或查找 `__mocks__` 文件。相对路径以调用所在的文件为基准解析。找不到对应的 mock 时，`rs.requireMock` 会同步抛出模块不存在的错误。
...
如果需要自动 mock，可以使用 `rs.mockRequire('./math.cjs', { mock: true })`。`rs.doMockRequire` 不会被提升，使用时需要在 `rs.requireMock` 之前调用。
