...
- 可多次调用以模拟多个环境变量。
- 使用 `rs.unstubAllEnvs()` 可恢复所有通过此方法更改的环境变量。
...
- 可多次调用以模拟多个全局变量。
- 使用 `rs.unstubAllGlobals()` 可恢复所有通过此方法更改的全局变量。
