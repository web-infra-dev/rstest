...
当你在 Browser Mode 测试里导入 `@rstest/browser`（例如导入 `page`）时，`expect` 会自动具备 `element` 能力。它提供 Playwright-style 的 Locator 断言体验；具体断言行为（例如自动重试）由你配置的 browser `provider` 决定。这让查询、交互、断言围绕同一 `Locator` 组织，减少手写轮询与时序抖动，失败信息也更聚焦于元素状态。
