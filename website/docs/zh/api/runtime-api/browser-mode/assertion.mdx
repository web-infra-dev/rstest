...
2. **当前测试的剩余时间**：未传 matcher timeout 时，断言使用当前测试剩余的时间。Browser Mode 测试的默认 `testTimeout` 为 15000ms。
3. **`expect.poll.timeout`**：当通过 `testTimeout: 0` 禁用当前测试超时时，使用该配置作为有限的 fallback。
