...
1. **Per-assertion timeout**：直接传给 matcher 的 `timeout` 参数，优先级最高，包括 `0`。使用 Playwright 时，显式传入的 `0` 会原样交给 provider，表示不设置 matcher 自身的超时。
...
在非并发流程中，未传 matcher timeout 时，实际断言超时取 `expect.poll.timeout` 与当前 test 或 hook 剩余时间中较小的值。Browser Mode 的 `5000ms` 默认值可以给异步 UI 更新更充足的时间，同时不会让错误断言等待完整的 `testTimeout`。显式传入的 matcher timeout 不参与这个计算，但外层 test 或 hook 的超时仍然可以结束断言。
