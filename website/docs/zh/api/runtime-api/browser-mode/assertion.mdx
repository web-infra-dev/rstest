...
3. **当前测试的剩余时间**：断言超时会受当前测试剩余时间限制，因此不会超过测试本身。Browser Mode 测试的默认 `testTimeout` 为 `15000ms`。

也就是说，未传 matcher timeout 时，实际断言超时取 `expect.poll.timeout` 与当前测试剩余时间中较小的值。Browser Mode 的 `5000ms` 默认值可以给异步 UI 更新更充足的时间，同时不会让错误断言等待完整的 `testTimeout`。
