...
如果你希望在同步的 `import` 语句中拿到真实实现，参考 [`importActualMethods`](../../config/test/importActualMethods) 中已经启用的同步的 `importActual` 方法。当启用了 `'importAttributes'` 时，下面的写法会在测试文件加载时直接引入未被 mock 的模块，再结合 `rs.mock` 进行常见的部分模块 mock：
