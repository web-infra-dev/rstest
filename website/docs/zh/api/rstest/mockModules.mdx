...
Rstest 支持对模块进行 mock （mock），这使得你可以在测试中替换模块的实现。Rstest 提供了 `rs` (`rstest`) 的工具函数来进行模块的 mock 。你可以直接使用以下方法来 mock 模块：

## rs.mock

- **类型：**: `<T = unknown>(moduleName: string, ,moduleFactory?: () => T) => void`

调用 `rs.mock` 时，Rstest 会将第一个参数传入的请求对应的模块进行 mock 替换。`rs.mock` 将根据是否提供了第二个 mock 工厂函数来决定如何处理被 mock 的模块，下面会详细介绍这两种情况。
...
无视一个模块是否被 mock，导入其原始的模块。如果你想部分 mock 模块，可以使用 `rs.importActual` 来获取原始模块的实现与 mock 的实现进行合并进行部分 mock。
