...
`forks` 仍然是默认值。大多数现有测试代码依赖按文件维度的进程隔离，而 threads 不提供这种隔离。当测试不会修改 `process.chdir`、环境变量或 signal handlers 时，可以切换至 `threads`。请参考 [pool](/config/test/pool) 了解更多。
...
该机制在 CI container 和资源受限的开发机器上最为关键。在此前仅按 CPU 数扩容的策略下，worker RSS 累积可能耗尽系统内存，或在多个 IPC payload 同时到达时使宿主进程触发 V8 heap-limit 崩溃。启用 gate 后，Rstest 会在每次新建 worker 前检查内存状态，并在判定启动下一个 worker 不安全时延迟其 spawn。

内存充足时没有任何公开 API 或默认行为变化，gate 在内存未承压的机器上完全不可见。如果需要禁用（例如做 benchmark、debug，或在严格隔离环境中），可以通过环境变量关闭：
...
运行时只会输出 `fail log`，`pass log` 会被丢弃。
...
重构后的 pool 也改为 spawn-on-demand：未使用的 worker 槽不会产生启动开销和内存占用。这也是 [基于内存的 spawn gate](#memory-gate) 得以实现的前提，同时也是新 [`threads` pool](#threads-pool) 实现的一部分。
