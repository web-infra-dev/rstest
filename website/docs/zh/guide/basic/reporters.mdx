...
1. **CLI**: `--reporters=<name>` (可多次使用；同时兼容 `--reporter` 作为别名)
...
| `summary`     | `boolean \| { maxCharsPerField?: number }` | `true` | 控制 Markdown 摘要输出和单字段长度。  |

<ApiMeta addedVersion="0.11.6" />

使用 `summary` 的对象形式，可以自定义每个失败消息和 diff，以及每个未处理错误消息和 stack 包含的最大字符数。`maxCharsPerField` 默认为 `400`。GitHub 将一个 step 生成的完整 Summary 限制为 [1 MiB](https://docs.github.com/zh/actions/writing-workflows/choosing-what-your-workflow-does/workflow-commands-for-github-actions#步骤隔离和限制)，因此应为该配置设置合理的上限。将 `summary` 设置为 `false` 可以禁用 Markdown 摘要。
...
Blob 报告器将测试结果序列化为 JSON 文件，以便后续合并。它专门用于 CI 环境中的[测试分片](/guide/basic/cli#sharding-tests)场景，在多台机器上拆分运行测试后需要合并结果。与分片一样，它不支持 watch 模式。
...
Blob 文件默认保存到 `.rstest-reports/` 目录。启用覆盖率时，每个 blob 只保存该次运行实际采集的覆盖率。支持延迟最终处理的 coverage provider 会让使用 blob 报告器的运行不生成覆盖率报告、不根据 `coverage.include` 补齐未测试文件，也不检查覆盖率阈值。未使用 blob 报告器的运行会正常完成覆盖率最终处理，包括分片运行。旧版 provider 保持原有的单次运行最终处理行为。所有生成 blob 的运行完成后，使用启用了覆盖率的 [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) 基于统一结果集中执行这些步骤。只传给生成 blob 的运行的覆盖率参数不会写入 blob；请把最终处理参数放在配置文件中，或在执行 `merge-reports` 时再次传入。

完整的 CI 工作流示例请参考[使用分片拆分测试](/guide/advanced/ci#split-tests-with-shards)。
