...
Blob 文件默认保存到 `.rstest-reports/` 目录。启用覆盖率时，每个 blob 只保存该分片运行期间实际采集的覆盖率。分片运行不会生成覆盖率报告、根据 `coverage.include` 补齐未测试文件或检查覆盖率阈值。所有分片完成后，使用启用了覆盖率的 [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) 基于统一结果集中执行这些步骤。
