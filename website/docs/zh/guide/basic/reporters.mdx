...
Blob 文件默认保存到 `.rstest-reports/` 目录。启用覆盖率时，每个 blob 只保存该分片运行期间实际采集的覆盖率。支持延迟最终处理的 coverage provider 会让分片运行不生成覆盖率报告、不根据 `coverage.include` 补齐未测试文件，也不检查覆盖率阈值。旧版 provider 保持原有的分片内最终处理行为。所有分片完成后，使用启用了覆盖率的 [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) 基于统一结果集中执行这些步骤。只通过分片命令传入的覆盖率参数不会写入 blob；请把最终处理参数放在配置文件中，或在执行 `merge-reports` 时再次传入。
