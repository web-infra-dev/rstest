---
description: Rstest 支持在单个 Rstest 进程中同时运行多个测试项目，这些项目可以有不同的测试配置和环境。
---
...
任何 `projects` 数组里的对象项，包括嵌套 `projects`，都建议使用 [defineInlineProject](/api/javascript-api/rstest-core#defineinlineproject)。[defineProject](/api/javascript-api/rstest-core#defineproject) 只用于项目配置文件的顶层导出。

两者最主要的区别在于 `name` 的处理方式。`defineProject` 允许顶层导出的 project 省略 `name`，Rstest 会按 [name](/config/test/name) 配置项的规则自动推导。`defineInlineProject` 则要求每个 inline project 都显式提供 `name`。

你可以通过 [defineProject](/api/javascript-api/rstest-core#defineproject) 辅助函数来定义 Project 配置：
...
你也可以在同一个 `projects` 列表里混合 Browser Mode 与 `testEnvironment: 'node'` 或 `'jsdom'` 项目，最终会合并输出测试结果。

### 查看配置

如果你希望查看 Rstest 最终生效的配置，可以通过 `DEBUG=rstest` 环境变量开启[调试模式](/guide/debug/debugging#开启调试模式)，Rstest 会将最终生效的 Rstest 配置及构建配置写入到产物目录下。
