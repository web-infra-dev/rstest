...
在大型项目中，你往往已经有构建/工具链的既有配置（别名、全局变量、插件等）。Rstest 通过 [适配器（Adapter）](/guide/advanced/adapters) 与 [extends](/config/test/extends) 选项，支持将这些外部配置转换并整合到测试配置中，从而避免重复维护、保持一致性。
