---
description: 本 E2E 测试指南介绍如何使用 @rstest/playwright，在运行于 Node.js worker 的 Rstest 测试中测试完整页面和应用。
---
...
E2E 测试（端到端测试）从用户的角度验证一个完整的页面或应用：打开页面、点击、输入，然后检查页面上真实呈现的结果。Rstest 通过 [@rstest/playwright](https://github.com/web-infra-dev/rstest/tree/main/packages/playwright) 集成 [Playwright](https://playwright.dev/) fixtures 和 Playwright 风格的断言。

测试代码运行在 Node.js worker 中，由 Playwright 驱动真实浏览器访问本地 dev server、preview server 或线上 URL，并和项目里的其他 Rstest 测试共用同一套 runner、配置和报告。如果要测的是单个组件而不是整个应用，请看 [浏览器模式](/guide/browser-testing)。
...
- [playwright](https://www.npmjs.com/package/playwright) 提供浏览器自动化运行时。
...
推荐从 `@rstest/playwright` 导入 `test` 和 `expect`，而不是从 `@rstest/core` 导入，这样可以获得 `toHaveTitle`、`toHaveText` 等 Playwright 专用断言能力：
...
### Playwright 类型 {#playwright-types}

<ApiMeta addedVersion="0.12.1" />

`@rstest/playwright` 会重新导出常用的 Playwright 类型，因此测试辅助函数可以和 `test`、`expect` 使用同一个入口：
...
这些仅类型导出包括 `APIRequestContext`、`APIResponse`、`Browser`、`BrowserContext`、`BrowserContextOptions`、`BrowserType`、`Frame`、`LaunchOptions`、`Locator`、`Page`、`Request`、`Response` 和 `Route`。

`@rstest/playwright` 是 Rstest 的集成层，而不是 `playwright` 包的替代品。它不会 re-export `chromium`、`firefox`、`webkit` 或 `request` 等运行时 API。如果确实需要这些 API，请直接从 `playwright` 导入：

```ts
import { chromium, request } from 'playwright';
```

一般的 E2E 测试直接使用 `browser`、`context`、`page` 和 `request` fixtures 即可，Rstest 会按照 fixture 生命周期创建并清理这些资源。

也可以从 `@rstest/playwright` 直接导入 `describe`、`beforeEach` 等生命周期方法：
...
可以通过 `definePlaywrightConfig` 在 `rstest.config.ts` 中设置 Playwright 默认选项：
...
`definePlaywrightConfig` 会为当前 project 配置默认的 `playwright` fixture。如果使用多 project 配置，请将它添加到每个使用 `@rstest/playwright` 的 Node.js project 的 `extends` 中。配置值必须支持 JSON 序列化，不支持 `launchOptions.logger` 等函数、类实例、客户端证书中直接传入的 `Buffer`，以及依赖当前测试或重试上下文的值。此类配置请改用 `certPath`/`keyPath`/`pfxPath` 或 `test.extend`。如果多个测试文件需要使用另一组选项，可以在共享模块中通过 `test.extend` 覆盖 fixture，并让测试文件统一从该模块导入 `test` 和 `expect`：
...
| Playwright Locator/Page 断言超时 | `expect.poll.timeout`（使用此 helper 时为 `5000ms`） |
...
这些默认值只在使用 `definePlaywrightConfig` 时生效。仅从 `@rstest/playwright` 导入 `test` 或 `expect` 不会改变 Rstest 的运行默认值。单元测试与 E2E 混用的 workspace 应只在 E2E project 中使用此 helper。
...
Hook 的作用域是所在的 `describe` 块，而不是某个扩展后的 test 对象。因此，该块内的每个测试都必须提供 hook 请求的 fixture；否则，Rstest 会在调用 hook 前让该测试失败，并报告缺失的 fixture。`afterEach` 和 `beforeEach` 返回的清理函数同样支持这一行为。在一次测试执行内，hook 与测试函数共享 fixture 实例，最后按 setup 的反序执行 teardown。

请在 hook 参数中通过直接对象解构声明 fixture 依赖。在 hook 函数体内再解构 context 对象不会请求 fixture。使用 fixture 的回调不支持剩余属性（rest property）和默认值。

### 在 `test.for` 中使用 fixture
...
第二个参数使用命名参数时，仍然可以访问 `task`、`expect` 等内置 `TestContext` API，但通过这个参数访问或解构属性不会请求 fixture。
...
Playwright Locator 和 Page 断言读取 Rstest 的 [`expect.poll.timeout`](/config/test/expect#expectpolltimeout)。配置一次即可同时控制这些断言和 `expect.poll()`：
...
matcher 的 `{ timeout }` 优先于 `expect.poll.timeout`，`.not` 和 `expect.soft` 也遵循这一规则。helper 将 `expect.poll.timeout` 的默认值设为 `5000ms`；未使用 helper 且未显式配置时，Node 模式默认使用 `1000ms`。即使不使用 helper，显式配置也能生效；组合多个 helper 时，也不再有另一套断言超时需要合并。
...
这种方式不会阻塞测试运行，同时可以把失败时的页面状态保留下来作为 artifact。完整的示例可以参考 [Rstest + Playwright 示例](https://github.com/rstackjs/rstack-examples/tree/main/rstest/playwright)，它使用 `@rstest/playwright` 测试一个 Rsbuild + React 项目。
...
| 场景                                              | 推荐方案                                                         |
| ------------------------------------------------- | ---------------------------------------------------------------- |
| 测试组件，并使用 Rstest 的 web 打包和浏览器运行时 | [Rstest 浏览器模式](/guide/browser-testing) 和 `@rstest/browser` |
| 通过 `page.goto()` 测试完整页面或应用             | `@rstest/playwright`                                             |
| 驱动已有 dev server、preview server 或线上 URL    | `@rstest/playwright`                                             |
| 需要浏览器内组件测试能力                          | [Rstest 浏览器模式](/guide/browser-testing)                      |
...
已有的 Playwright Test 项目可以交给支持 Skills 的 Coding Agent 迁移，使用 [migrate-to-rstest](https://github.com/rstackjs/agent-skills#migrate-to-rstest) skill。Agent Skills 是安装到 Coding Agent 里的领域知识包，让 Agent 在特定场景下给出更准确的建议或直接执行操作，[skills](https://www.npmjs.com/package/skills) 包负责安装它们。`migrate-to-rstest` skill 包含 Playwright 配置、fixtures 和行为一致性的迁移指南。
...
<Prompt
  title="将 Playwright 迁移到 @rstest/playwright"
