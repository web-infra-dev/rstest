...
| `--teardownTimeout <value>`         | 设置运行结束后等待进程退出的时间（毫秒）。设为 0 可立即退出。详见 [teardownTimeout](/config/test/teardown-timeout)。                                                     |
...
| `--pool.memoryLimit <limit>`        | worker 回收阈值：`forks` 在 `isolate: false` 时按 RSS 判断，VM pools 按 V8 堆用量判断，详见 [pool](/config/test/pool#poolmemorylimit)                                    |
