---
description: Rstest 提供了一个轻量级的命令行工具，包含 rstest watch 和 rstest run 等命令。
---
...
- 合并所有分片实际采集的覆盖率（如果启用了覆盖率）
- 统一应用一次 `coverage.include`，补齐所有分片都未测试的文件
- 基于统一结果生成覆盖率报告并检查阈值

支持延迟最终处理的 coverage provider 会让使用 blob 报告器的分片运行有意跳过覆盖率报告生成和阈值检查。旧版 provider 保持原有的分片内最终处理行为。请通过配置或 `--coverage` 参数为 `merge-reports` 启用覆盖率，让合并 job 完成覆盖率最终处理。合并命令支持与 `rstest run` 相同的 `--coverage.*` 参数。只传给分片的覆盖率参数不会写入 blob，因此请在配置文件中定义 `coverage.include`、`coverage.reporters` 和 `coverage.reportsDirectory` 等最终处理参数，或在执行 `merge-reports` 时再次传入。
...
| `--coverage.allowExternal`          | 收集项目根目录之外文件的覆盖率，详见 [coverage.allowExternal](/config/test/coverage#allowexternal)                                                                       |
| `--coverage.changed [commit]`       | 只收集当前 Git 仓库中变更文件的覆盖率，也可以指定从某个 commit 以来的变更                                                                                                |
| `--coverage.clean`                  | 测试前清理覆盖率目录，详见 [coverage.clean](/config/test/coverage#clean)                                                                                                 |
| `--coverage.enabled`                | 启用代码覆盖率收集，详见 [coverage.enabled](/config/test/coverage#enabled)                                                                                               |
| `--coverage.exclude <pattern>`      | 指定需要排除覆盖率收集的文件，详见 [coverage.exclude](/config/test/coverage#exclude)                                                                                     |
| `--coverage.include <pattern>`      | 指定需要收集覆盖率的文件，详见 [coverage.include](/config/test/coverage#include)                                                                                         |
| `--coverage.provider <provider>`    | 选择覆盖率 provider（`istanbul` \| `v8`），详见 [coverage.provider](/config/test/coverage#provider)                                                                      |
| `--coverage.reporters <reporter>`   | 指定一个或多个覆盖率报告器，详见 [coverage.reporters](/config/test/coverage#reporters)                                                                                   |
| `--coverage.reportOnFailure`        | 测试失败时仍生成覆盖率报告，详见 [coverage.reportOnFailure](/config/test/coverage#reportonfailure)                                                                       |
| `--coverage.reportsDirectory <dir>` | 指定覆盖率报告目录，详见 [coverage.reportsDirectory](/config/test/coverage#reportsdirectory)                                                                             |
| `--dev.writeToDisk`                 | 将测试临时文件写入磁盘，详见 [dev.writeToDisk](/config/build/dev#devwritetodisk)                                                                                         |
...
| `--includeTaskLocation`             | 收集测试和 suite 的位置信息，详见 [includeTaskLocation](/config/test/include-task-location)                                                                              |
...
| `-f, --onlyFailures`                | 仅重新运行上一次运行中失败的测试文件，详见 [onlyFailures](/config/test/only-failures)                                                                                    |
| `--output.cleanDistPath`            | 测试开始前清理测试临时文件，详见 [output.cleanDistPath](/config/build/output#outputcleandistpath)                                                                        |
| `--output.emitAssets`               | 输出导入的静态资源，详见 [output.emitAssets](/config/build/output#outputemitassets)                                                                                      |
| `--output.module`                   | 以 ES module 格式输出 JavaScript 文件，详见 [output.module](/config/build/output#outputmodule)                                                                           |
...
| `--reporters, --reporter <name>`    | 指定一个或多个测试报告器，详见 [reporters](/config/test/reporters)                                                                                                       |
...
| `--source.tsconfigPath <path>`      | 指定 tsconfig.json 文件路径，详见 [source.tsconfigPath](/config/build/source#sourcetsconfigpath)                                                                         |
...
| `--trace`                           | 导出 Perfetto 兼容的性能 trace JSON 文件，并附带一份按耗时排序的 markdown 摘要（打印到终端并写入 trace 同目录），详见 [使用 --trace](/guide/debug/profiling#using-trace) |
...
| `--config-loader <loader>` | 指定配置加载器 （`auto` \| `jiti` \| `native`），详见 [Rsbuild - 指定加载方式](https://rsbuild.rs/guide/configuration/rsbuild#specify-config-loader) |
| `-r, --root <root>`        | 指定项目根目录，详见 [root](/config/test/root)                                                                                                       |
| `--coverage`               | 合并完成后生成覆盖率报告，详见 [coverage](/config/test/coverage)                                                                                     |
| `--reporters <name>`       | 指定在合并结果上运行的一个或多个报告器，详见 [reporters](/config/test/reporters)                                                                     |
| `--cleanup`                | 合并完成后删除 blob 报告目录                                                                                                                         |
| `-h, --help`               | 显示帮助信息                                                                                                                                         |
