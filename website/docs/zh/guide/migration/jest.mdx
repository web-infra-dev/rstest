...
Jest 本身通过 `moduleNameMapper` 和 `transform` 配置模块解析和转换，而不是读取 `tsconfig.json`。如果你使用了 `ts-jest`，不要假设其中的 TypeScript 转换设置会自动迁移：直接配置 Rstest 时，它会自动应用 `compilerOptions.paths` 进行解析，但 `target`、装饰器语法等设置需要显式配置。adapter 可能会继承或推断额外设置，请参考所用 [adapter 的文档](/guide/integration/adapters)。详见 [source.tsconfigPath](/config/build/source#sourcetsconfigpath)。
