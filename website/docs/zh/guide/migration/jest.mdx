...
Jest 本身通过 `moduleNameMapper` 和 `transform` 配置模块解析和转换，而不是读取 `tsconfig.json`。如果你使用了 `ts-jest`，不要假设其中的 TypeScript 转换设置会自动迁移：Rstest 会自动应用 `compilerOptions.paths` 进行解析，但 `target`、装饰器语法等设置需要显式配置。详见 [source.tsconfigPath](/config/build/source#sourcetsconfigpath)。
