...
| `vmMemoryLimit`               | [`pool.memoryLimit`](/config/test/pool)                                                        | 将 Vitest `vmThreads` 迁移到 Rstest `vmThreads` 时使用。两者都会在文件结束后按阈值回收 VM worker；Rstest 还会用该值限制每个 worker 的不可变资源与编译缓存。                                                                                                                                                                                                                                                                                                                                                                 |
