...
Rstest 暂不支持文件级 `@vitest-environment` 注释。如果你需要按文件切换环境，当前建议用不同的 project 或不同的配置文件来拆分。更多自定义 environment API 说明请参考 [`testEnvironment`](/config/test/test-environment#自定义-environment)。
