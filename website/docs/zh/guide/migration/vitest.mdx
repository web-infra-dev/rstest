...
从 Vitest 自定义 environment 迁移时，对象形态更接近，但 `viteEnvironment` 需要迁移到 Rstest 配置中的 `transformMode`：
...
将 `viteEnvironment: 'ssr'` 映射为 `transformMode: 'node'` 或省略 `transformMode`，将 `viteEnvironment: 'client'` 映射为 `transformMode: 'browser'`。对于 `vitest-environment-foo` package，可以使用完整 package 名，或发布 / 重命名为 `rstest-environment-foo`。
