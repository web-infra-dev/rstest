...
| `isolate`                     | [`isolate`](/config/test/isolate)                                                              | `vmThreads` 始终为每个文件创建新的 VM context，因此该 pool 会接受 `isolate: false`，但不会因此关闭文件隔离。                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `poolOptions.forks.maxForks`  | [`pool.maxWorkers`](/config/test/pool)                                                         | Rstest 把 `poolOptions` 摊平到顶层 `pool`：`poolOptions.forks.maxForks` → `pool.maxWorkers`、`execArgv` → `pool.execArgv`。`minForks` 没有对应项 —— Rstest 不提供 `minWorkers`。Rstest 支持 `forks`、`threads` 和 `vmThreads`；`vmForks` 没有对应项，会回退到 `forks`。Vitest 4 的顶层 `test.maxWorkers` 映射到 `pool.maxWorkers`（`test.minWorkers` 会被丢弃）。                                                                                                                                                           |
| `vmMemoryLimit`               | [`pool.vmMemoryLimit`](/config/test/pool)                                                      | 将 Vitest `vmThreads` 迁移到 Rstest `vmThreads` 时使用。两者都会在文件结束后按阈值回收 VM worker；Rstest 还会用该值限制每个 worker 的不可变资源与编译缓存。                                                                                                                                                                                                                                                                                                                                                                 |
