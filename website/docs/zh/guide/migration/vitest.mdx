...
| ----------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment`                 | [`testEnvironment`](/config/test/test-environment)                                             | 将 `test.environmentOptions` 合并到对象形式：`testEnvironment: { name: 'jsdom', options: { ... } }`。Rstest 支持通过 Rstest environment 对象加载自定义 environment 包和 JavaScript 文件路径，包名解析时也会尝试 `rstest-environment-*`。自定义 environment 的迁移细节请参考 [从 Vitest 迁移自定义 environment](#custom-environments)；`@vitest-environment` 文件级注释会在内置 environment 场景下被识别。                                                                                                                   |
...
从 Vitest 自定义 environment 迁移时，对象形态更接近，但 `viteEnvironment` 需要迁移到 Rstest 配置中的 `target`：
...
将 `viteEnvironment: 'ssr'` 映射为 `target: 'node'` 或省略 `target`，将 `viteEnvironment: 'client'` 映射为 `target: 'web'`。对于 `vitest-environment-foo` package，可以使用完整 package 名，或发布 / 重命名为 `rstest-environment-foo`。

更多自定义 environment API 说明请参考 [`testEnvironment`](/config/test/test-environment#自定义-environment)。
