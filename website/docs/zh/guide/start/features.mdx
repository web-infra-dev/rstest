---
description: 了解 Rstest 面向构建一体化测试提供的能力，包括 Rstack 配置复用、浏览器模式、Playwright 驱动的 E2E 测试、测试分片和覆盖率。
---
...
了解更多关于 [Module Federation 测试](/guide/integration/module-federation)。
...
了解更多关于 [Playwright](/guide/integration/playwright)。

## 代码覆盖率

Rstest 同时支持 [istanbul](https://istanbul.js.org/) 和 `v8` 两种覆盖率 provider。你可以在 Rstest 配置文件中将 `coverage.enabled` 设置为 `true` 来启用覆盖率，再根据项目需求选择对应的 provider。

如果你需要基于 SWC 插桩且语义稳定的 Istanbul coverage，可以使用 `istanbul`；如果 coverage 的主要瓶颈是内存占用，可以使用 `v8`。在 headless、非 watch 的 Chromium Browser Mode 运行中，`v8` 从浏览器原生收集 coverage，无需对应用代码插桩。headed 和 watch coverage 仍为实验性能力；Firefox 和 WebKit 使用 `istanbul`。
