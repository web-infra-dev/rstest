...
如果你需要基于 SWC 插桩且语义稳定的 Istanbul coverage，可以使用 `istanbul`；如果 coverage 的主要瓶颈是内存占用，可以使用 `v8`。在 Chromium Browser Mode 中，`v8` 从浏览器原生收集 coverage，无需对应用代码插桩；Firefox 和 WebKit 使用 `istanbul`。
