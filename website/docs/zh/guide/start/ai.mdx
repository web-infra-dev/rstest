...
当你希望 AI 直接消费测试结果时，可以使用 `md` 报告器。它会将测试结果输出为一份 markdown 文档到 stdout，相比带颜色的终端日志更容易被 LLM 解析。
...
完整选项（`preset`、`failures`、`stack`、`console`）请参考 [Markdown reporter](/guide/basic/reporters#markdown-报告器)。
