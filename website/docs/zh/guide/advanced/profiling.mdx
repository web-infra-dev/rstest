...
[Samply](https://github.com/mstange/samply) 支持同时对 Rstest 主进程和测试进程进行性能分析。

### 使用 `--profile` 快速上手

在 rstest 命令上加 `--profile`，rstest 会替你解析好 entry 路径并打印一条可直接执行的 samply 命令：

```bash
rstest --profile
```

如果已安装 samply，rstest 会直接打印命令供你复制运行；如果未安装，rstest 会输出安装指引（`cargo install samply`，macOS 也可以 `brew install samply`），并附带安装后可执行的命令。

我们刻意不让 rstest 直接 spawn samply——只解析并打印命令，这样 signal、exit code、shell history 等都仍然由 samply 自己接管。

### 手动执行

也可以不通过 `--profile`，直接运行 samply：
