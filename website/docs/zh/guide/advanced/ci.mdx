...
- [@rstest/coverage-v8](https://github.com/web-infra-dev/rstest/tree/main/packages/coverage-v8)：基于 V8 的 provider，适用于 Node mode 和 Chromium Browser Mode。
...
每个分片会写入 `.rstest-reports/blob-{index}-{count}.json`。合并 job 会把这些文件收集到同一个 `.rstest-reports/` 目录，然后 `rstest merge-reports` 会基于统一结果运行已配置的 reporter，并合并覆盖率数据。更多细节请查看 [测试分片](/guide/basic/cli#sharding-tests) 和 [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports)。
