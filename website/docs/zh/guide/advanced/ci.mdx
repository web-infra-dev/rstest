...
可以在 `rstest.config.ts` 中配置 [@rstest/playwright](/integration/playwright) 的默认启动选项：
...
使用该 fixture 后，标准 GitHub-hosted workflow 只需要安装 npm 依赖并运行 Rstest，不要再添加 `playwright install` 步骤。fixture 用法和 trace artifact 请查看 [@rstest/playwright 指南](/integration/playwright)。
