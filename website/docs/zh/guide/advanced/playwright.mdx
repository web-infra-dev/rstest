...
从 `@rstest/playwright` 导入 `test`。如果使用 `toHaveTitle`、`toHaveText` 等 Playwright 专用断言，也要从 `@rstest/playwright` 导入 `expect`。这个 `expect` 会为 `Page` 和 `Locator` 增加对应的断言，但不会替换全局的 Rstest `expect`。即使启用 `globals: true`，全局 `expect` 仍然是 core 的 Rstest expect：
...
`definePlaywrightConfig` 会添加一个生成的 setup file，用来初始化默认的 `playwright` fixture。如果使用多 project 配置，请将它添加到每个使用 `@rstest/playwright` 的 Node.js project 的 `extends` 中。配置值必须支持 JSON 序列化，不支持 `launchOptions.logger` 等函数、class instance、client certificate 中直接传入的 `Buffer`，以及依赖当前测试或 retry context 的值。此类配置请改用 `certPath`/`keyPath`/`pfxPath` 或 `test.extend`。如果多个测试文件需要使用另一组选项，可以在共享模块中通过 `test.extend` 覆盖 fixture，并让测试文件统一从该模块导入 `test` 和 `expect`：
