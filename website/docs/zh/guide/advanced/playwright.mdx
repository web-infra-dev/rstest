...
## 用 Midscene 编写 AI 驱动的测试

UI 一变，selector 就会失效。[Midscene](/guide/advanced/midscene) 构建在 `@rstest/playwright` 之上，让你改用自然语言描述交互和断言，并生成可视化 HTML 报告，带着截图回放每一步 AI 操作：
...
本页的所有 fixture 和选项依然可用，你可以在同一个测试里混用 `agent` 调用和原生 Playwright API。
