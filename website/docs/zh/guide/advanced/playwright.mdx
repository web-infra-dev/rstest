...
如果要将现有的 Playwright Test 项目迁移到 `@rstest/playwright`，可以让支持 Skills 的 Coding Agent 使用 [migrate-to-rstest](https://github.com/rstackjs/agent-skills#migrate-to-rstest) skill 辅助完成迁移。先安装该 skill：
...
<Prompt
  ...
  prompt={`Migrate this Playwright Test project to @rstest/playwright by following the migrate-to-rstest skill. Preserve existing test behavior and coverage, migrate the Playwright Test configuration and fixtures, and verify test discovery and results after the migration.`}
/>
