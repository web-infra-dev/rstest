...
该配置会应用到当前 project 的默认 `playwright` fixture。配置值必须可以通过 worker transport 传递。如果多个测试文件需要使用另一组选项，可以在共享模块中通过 `test.extend` 覆盖 fixture，并让测试文件统一从该模块导入 `test` 和 `expect`：
