---
description: 适配器是一个强大的功能，它允许你将来自其他工具（如构建工具或 CLI）的配置转换为 Rstest 支持的配置格式。通过创建一个自定义适配器，你可以复用现有配置，避免在多个工具之间重复定义，并确保项目的一致性。
---
...
- [Rslib Adapter](/integration/rslib#安装适配器)：用于从 Rslib 配置中扩展 Rstest 配置。
- [Rsbuild Adapter](/integration/rsbuild#安装适配器)：用于从 Rsbuild 配置中扩展 Rstest 配置。
- [Rspack Adapter](/integration/rspack#安装适配器)：用于从 Rspack 配置中扩展 Rstest 配置。
