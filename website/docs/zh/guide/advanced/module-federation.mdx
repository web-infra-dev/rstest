...
Browser Mode 下插件不会替你打开 `federation` 配置项。只要项目用了 `globalSetup`，就要自己写上 `federation: true`：setup 文件在 Node worker 里执行，用的是同一份模块联邦构建，没有这个开关时模块联邦运行时会在文件加载阶段抛错，生产者服务永远起不来。
