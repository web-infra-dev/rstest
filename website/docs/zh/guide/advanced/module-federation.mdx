...
[@module-federation/rstest](https://github.com/module-federation/core) 为 Rstest 构建配置模块联邦。它支持 Node 和 JSDOM 测试环境以及 Browser Mode，因此同一个测试项目可以覆盖服务端与浏览器端的消费者。

## 安装

将 `@module-federation/rstest` 添加到现有 Rstest 项目。Node 和 JSDOM 测试需要 `@rstest/core@0.11.4` 或更高版本，Browser Mode 需要 `@rstest/core@0.11.10` 或更高版本。
...
- **HTTP 生产者。** 通过 HTTP 提供的生产者写成 `remote@http://localhost:3001/remoteEntry.cjs` 这样的地址，并设置 `remoteType: 'script'`。服务要在测试运行前启动，见[用 `globalSetup` 启动生产者](#serve-remotes)。
...
## 用 `globalSetup` 启动生产者 \{#serve-remotes}
...
Browser Mode 下插件不会自动打开 `federation` 配置项。但 `globalSetup` 文件始终在 Node 进程里执行，产物里同样带着模块联邦运行时，没有这个开关它一加载就会报错，生产者服务也就起不来。所以 Browser Mode 项目只要用了 `globalSetup`，就要手动写上 `federation: true`。
