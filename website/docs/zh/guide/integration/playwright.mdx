...
对于 Playwright E2E 项目，建议在 `rstest.config.ts` 中设置 `isolate: false`。这样同一个测试文件里的测试会在同一个 worker 中运行，`@rstest/playwright` 可以在文件结束时统一清理 browser、context、page 和本地 server：
