...
如果只是临时用命令行风格打开调试，不想修改测试代码，可以使用 `--playwright.trace`。这里刻意使用 provider 作用域，因为现有顶层 `--trace` 已经表示 Rstest 性能追踪：

```bash
rstest --playwright.trace=retain-on-failure
```

可以用 `--playwright.trace.outputDir` 覆盖默认输出目录：

```bash
rstest --playwright.trace=on --playwright.trace.outputDir=.rstest/playwright-traces
```

fixture 选项和 `RSTEST_PLAYWRIGHT_TRACE` 环境变量仍按上面的优先级生效。`trace` 支持 `'off'`、`'on'`、`'retain-on-failure'`，也支持传入 options 对象：
...
默认情况下，trace 会写入 `.rstest/playwright-traces/<test-id>/`。每次保存 trace 会包含：

- `trace.zip`：Playwright 官方 trace 产物。可以用 `pnpm exec playwright show-trace <path-to-trace.zip>` 打开。
