...
Rstest browser mode 的目标场景不同，更偏向组件测试和浏览器内测试。它会通过 Rstest 对测试文件和源码进行 web 打包，并在浏览器运行时中执行测试代码。
...
对于 Playwright E2E 项目，建议在 `rstest.config.ts` 中设置 `isolate: false`，这样 worker 级别的 browser 和 server 状态可以在测试文件间稳定复用：
