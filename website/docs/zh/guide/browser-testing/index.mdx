...
| [isolate](/config/test/isolate)                                                                  | `true` 时每个文件使用全新的浏览器 context/page。设置为 `false` 时，headless browser worker 会在分配给它的文件间复用 context/page，因此 worker scope fixture 可以跨文件存活；headed 模式仍保持可见文件 frame 隔离。启用 `bail` 时文件会逐个执行，worker scope fixture 不会跨文件复用。        |
