...
模块 mock（[rs.mock 系列](/api/runtime-api/rstest/mock-modules)）与 [includeSource](/config/test/include-source) 源码内测试的行为与 Node 模式一致。`rs.mockRequire` 在浏览器测试中同样可用，但它面向 CommonJS 互操作场景——编写浏览器测试时优先使用 `rs.mock` / `rs.doMock`。
