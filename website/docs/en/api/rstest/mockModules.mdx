...
If you want to sync-import the actual implementation, refer to [`importActualMethods`](../../config/test/importActualMethods) for the supported synchronous `importActual` options. When `'importAttributes'` is enabled, the following pattern loads the unmocked module as the test file executes and lets you combine it with `rs.mock` for partial mocks:
