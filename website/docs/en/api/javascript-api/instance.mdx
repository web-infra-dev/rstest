...
Use [`loadConfig`](/api/javascript-api/rstest-core#loadconfig) from the main entry to load a config file, then pass its return value (`{ content, filePath, dependencies }`) directly as `config`.
...
`rstest.run()` runs one test cycle and returns a `TestRunResult`. It rejects only when the run cannot start at all (invalid options or configuration, or a node project's dev server that could not start). Everything after that — a browser project's dev server or browser that could not start, a `globalSetup` that threw, a compile error, or failing tests — is reported through `status` without rejecting the promise; see [TestRunResult](#testrunresult).
...
The remaining fields (`results`, `testResults`, `summary`, `duration`, `snapshotSummary`, `unhandledErrors`, `coverage`, `rerunTestPaths`) are documented under [`onTestRunEnd`](/api/javascript-api/reporter#ontestrunend) in the Reporter API.
...
- **Startup failure** — the config phase threw, or a node project's dev server could not start. `run()` and `watch()` both reject.
- **Fatal failure after startup** — a browser project's dev server or browser could not start, a first-cycle `globalSetup` threw or failed to compile, or the bundler ended a compile in a way that reattaches no file watcher. `run()` resolves with `status: 'error'`. In watch, the failed cycle still reaches `onResult`, then the session closes; if `watch()` has not resolved yet, it rejects with that error — several errors as an `AggregateError`.
- **Recoverable cycle failure** — a test failure, a test file that failed to compile, or a later-cycle `globalSetup` failure. The cycle reports the failure and the watch session stays open for the next trigger.
