...
A rerun execution error is delivered through `onResult` with `status: 'error'` and does not close the session; startup failures reject `watch()`.

Failures come in three kinds:

- **Startup failure** — nothing ran: the config phase threw, or a resource the run depends on could not start (a dev server, a browser). `run()` and `watch()` both reject.
- **Fatal failure after startup** — the bundler ended a compile in a way that reattaches no file watcher. `run()` resolves with `status: 'error'`. In watch the failed cycle still reaches `onResult`, the session then closes, and `watch()` rejects when it had not resolved yet.
- **Retryable failure** — a `globalSetup` that threw or failed to compile, or a test file that failed to compile. The cycle is `status: 'error'`, `run()` resolves, and the watch session stays open and retries on the next trigger.
