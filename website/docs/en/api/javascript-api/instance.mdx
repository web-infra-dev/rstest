...
```ts
type TestRunStatus = 'passed' | 'failed' | 'error';
...
- **Startup failure** — the config phase threw, or a resource the run depends on could not start (a dev server, a browser). `run()` and `watch()` both reject. A `globalSetup` that throws or fails to compile in the first watch cycle also rejects `watch()`, after delivering that cycle through `onResult` with `status: 'error'`. In non-watch mode, a `globalSetup` failure remains a `status: 'error'` result from `run()`.
- **Fatal failure after startup** — the bundler ended a compile in a way that reattaches no file watcher. `run()` resolves with `status: 'error'`. In watch, the failed cycle still reaches `onResult`, then the session closes; if `watch()` has not resolved yet, it rejects.
