...
`RunReport` and `FailureItem` are available as public type exports starting in Rstest 0.10.2.

- `RunReport`: The run-level summary passed to `Reporter.onTestRunEnd`, including the precomputed `status`, `counts`, `failures`, and `unhandledErrors`.
- `FailureItem`: A single failed case or file paired with its errors.
