...
1. **Per-assertion timeout**: The `timeout` parameter passed directly to the matcher, highest priority. This includes `0`; with Playwright, an explicit `0` is passed through as the provider's no-deadline value.
...
Outside concurrent flows, with no per-assertion override, the effective timeout is the smaller of `expect.poll.timeout` and the remaining active test or hook timeout. The Browser Mode `5000ms` default gives asynchronous UI updates more time without making a failed assertion wait for the full `testTimeout`. An explicit matcher timeout is not capped by this calculation; the enclosing test or hook timeout can still end the operation.
