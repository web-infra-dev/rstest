...
2. **Remaining test timeout**: When no matcher timeout is provided, the assertion uses the remaining time of the current test. Browser Mode tests default to 15000ms.
3. **`expect.poll.timeout`**: Used as the fallback when the current test timeout is disabled with `testTimeout: 0`.
...
```ts
// Uses the remaining time of the current test (default 15000ms in Browser Mode)
...
// With testTimeout: 0, element assertions still use a finite poll timeout
// (1000ms by default).
```
