...
3. **Remaining test timeout**: The assertion timeout is capped by the remaining time of the current test, so it cannot outlive the test. Browser Mode tests default to `15000ms`.

In other words, with no per-assertion override, the effective timeout is the smaller of `expect.poll.timeout` and the remaining test timeout. The Browser Mode `5000ms` default gives asynchronous UI updates more time without making a failed assertion wait for the full `testTimeout`.
