---
title: Hooks
description: Hooks allow you to run setup and teardown logic before or after your tests or test suites.
---
...
`ctx.meta` is added in 0.11.1. It is the mutable metadata object for the current suite. If the suite was declared with `describe(name, { meta }, fn)`, `ctx.meta` starts with that metadata after suite inheritance has been applied. File-level hooks write metadata to `TestFileResult.meta`; hooks inside a `describe` block write metadata to that suite result passed to custom reporters.
