...
| `'worker'` | Once per worker, then cleaned up when the worker finishes. With `isolate: false`, it can be shared across files. | Worker fixtures only.                  |
...
File- and worker-scoped fixtures use the builder form. It accepts `name`, `options`, and the fixture in that order. Its fixture function returns the value and can register one cleanup with `onCleanup`:
...
Omit the options argument to create a test-scoped builder fixture, as in `.extend('client', fixture)`. The object form remains available for test-scoped fixtures using the `await use(value)` lifecycle. When overriding a file- or worker-scoped fixture, repeat the same `scope`; changing or omitting its scope is an error.

Setup is lazy unless `auto: true` is set. Automatic file and worker fixtures are initialized before suite hooks in the first relevant file, and still run only once at their corresponding scope boundary.
