...
The object form uses the same `await use(value)` lifecycle with a tuple option:
...
The builder form accepts `name`, `options`, and the fixture in that order. Its fixture function returns the value and can register one cleanup with `onCleanup`:
...
Omit the options argument to create a test-scoped builder fixture, as in `.extend('client', fixture)`. Setup is lazy unless `auto: true` is set. An automatic file or worker fixture still runs only once at its corresponding scope boundary.
