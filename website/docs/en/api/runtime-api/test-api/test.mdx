...
Omit the options argument to create a test-scoped builder fixture, as in `.extend('client', fixture)`. The object form remains available for test-scoped fixtures using the `await use(value)` lifecycle. Setup is lazy unless `auto: true` is set. An automatic file or worker fixture still runs only once at its corresponding scope boundary.
