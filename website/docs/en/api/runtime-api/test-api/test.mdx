...
- `meta?: Record<string, unknown>` — initial metadata for the test result. If the test is inside a `describe` with `meta`, it inherits the suite metadata and test-level keys override inherited keys.
...
Use `context.task.meta` to attach JSON-serializable metadata to the current test result. Custom reporters and the programmatic API can read this metadata from `TestResult.meta`:
