...
- **Type:** `test.extend(fixtures: Fixtures) | test.extend(name, fixture) | test.extend(name, options, fixture)`
...
A test-scoped fixture function in the object form receives two parameters:
...
The first parameter of a test-scoped fixture function also includes `TestContext`, so you can read current test information or use `expect` directly inside a fixture:
...
Use the `scope` option to control how long a fixture instance lives:

| Scope      | Setup and cleanup boundary                                                                                                                                                         | Available dependencies                 |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| `'test'`   | Once per test attempt, including each retry. This is the default.                                                                                                                  | Test, file, worker, and `TestContext`. |
| `'file'`   | Once per test file, then cleaned up after all tests and suite hooks in that file.                                                                                                  | File and worker fixtures.              |
| `'worker'` | Once per worker, then cleaned up when the worker finishes. With `isolate: false`, it can be shared across files. A watch rebuild replaces the worker before running the new build. | Worker fixtures only.                  |

File- and worker-scoped fixtures cannot access `TestContext`, because there is no single current test for their full lifetime. A fixture can depend only on fixtures whose lifetime is at least as long as its own. Rstest reports an error for dependencies such as a worker fixture consuming a file or test fixture.

File and worker scopes are available through the builder form, which accepts `name`, `options`, and the fixture in that order. Its fixture function returns the value and can register one cleanup with `onCleanup`:

```ts
const apiTest = test
  .extend('port', { scope: 'worker' }, 5000)
  .extend('server', { scope: 'file' }, async ({ port }, { onCleanup }) => {
    const server = await startServer(port);
    onCleanup(() => server.close());
    return server;
  })
  .extend('client', async ({ server }) => createClient(server.url));
```

Define file- and worker-scoped fixtures at the top level of the test file. They are initialized lazily when a test, hook, or another fixture requests them. These scopes do not support `auto` and cannot override an existing fixture. These restrictions keep their dependency graph stable for the full file or worker lifetime.

Omit the options argument to create a test-scoped builder fixture, as in `.extend('client', fixture)`. The object form also always defines test-scoped fixtures. Its tuple options recognize `auto`, not `scope`; an array such as `[value, { scope: 'file' }]` remains an ordinary test-scoped fixture value.
...
Fixtures are lazy by default: they only run when requested through object destructuring by a test or per-test hook callback (or required by another fixture). To make a test-scoped fixture run for every test automatically — even when no callback requests it — use the tuple syntax with `{ auto: true }`:
