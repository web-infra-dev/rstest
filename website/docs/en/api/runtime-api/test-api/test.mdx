...
### Object-form fixture lifecycle

An object-form fixture function receives two parameters:
...
### Named builder fixtures

<ApiMeta addedVersion="0.11.6" />

The named builder form returns a fixture value directly instead of calling `use`. Its second parameter provides `onCleanup`, which registers one callback to run after the test and its per-test hooks finish. Both the builder and cleanup callback may be asynchronous.
...
Named builders are test-scoped: Rstest creates a new value for each test attempt and runs its cleanup before starting the next attempt. Use direct object destructuring in the first parameter to request other fixtures or fields from `TestContext`.
