---
title: Expect
description: expect is used to assert values in tests. It provides a rich set of matchers, supports chainable modifiers, soft assertions, polling, and snapshot testing.
overviewHeaders: [2, 3]
---
...
- **Default `timeout`:** `1000` (ms), or `5000` (ms) in Browser Mode

You can change these defaults with the [`expect.poll` configuration](/config/test/expect#expectpoll). Options passed to an individual call take precedence.
...
- `expect.poll(...)` does not support `resolves`/`rejects`, `toThrow*`, or snapshot matchers. Use `rs.waitFor()` for those unstable conditions.
...
Mock matchers verify interactions with `rs.fn` / `rs.spyOn`: calls, call order, return values, and async outcomes.
...
```ts
const sum = rs.fn((a: number, b: number) => a + b);
sum(1, 2);
sum(2, 3);
...
const fetchUser = rs.fn(async (id: number) => ({ id, name: 'alice' }));
await fetchUser(1);
await fetchUser(2);
