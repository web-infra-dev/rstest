...
`TestOptions` accepts the same fields as on [`test`](/api/runtime-api/test-api/test#testoptions): `timeout`, `retry`, `repeats`, and `meta`. These propagate to descendant suites and tests as **inheritable defaults**:
...
- Metadata is shallow-merged from parent suite to child suite or test; child metadata overrides keys from the parent.
