...
- Automatically called before each test if the [`unstubEnvs`](/config/test/unstub-envs) config is enabled.
...
- Automatically called before each test if the [`unstubGlobals`](/config/test/unstub-globals) config is enabled.
...
Retrieves the current runtime configuration for the test file. The `playwright` value contains the project-level options passed to the default fixture from `@rstest/playwright`.

**Example:**

```ts
const config = rs.getConfig();
console.log(config.playwright);
```
