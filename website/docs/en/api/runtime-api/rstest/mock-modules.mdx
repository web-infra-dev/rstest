...
Loads the mocked exports of a module asynchronously. To load a CommonJS mock synchronously, use [`rs.requireMock`](#rsrequiremock).

The result depends on how the module is mocked:

- If a mock has been registered with `rs.mock` or `rs.doMock`, Rstest returns that mock, including a factory result, a manual mock, or an auto-mocked module.
- If no mock is registered and the argument is a string literal, Rstest loads the matching `__mocks__` file when one exists. This does not require a preceding `rs.mock` call.
- If neither is available, the returned Promise rejects with a module-not-found error.

A registered factory takes precedence over a matching `__mocks__` file. Its exports are returned without automatically replacing its functions with `rs.fn()`. To create an auto-mocked module, register it with `rs.mock('./sum', { mock: true })` before loading it.
...
test('loads the registered mock', async () => {
  const mocked = await rs.importMock<typeof import('./sum')>('./sum');

  expect(mocked.sum(1, 2)).toBe(100);
});
```

### Variable paths

In Node and Browser mode, `rs.importMock` attempts to find a registered mock when given a variable path. You can register the target mock using a literal path in `rs.mock` or `rs.doMock`, then retrieve it through a variable path. Relative paths are resolved from the file containing the `rs.importMock` call.

```ts title="src/sum.test.ts"
import { expect, rs, test } from '@rstest/core';

test('loads an auto-mocked module through a variable', async () => {
  rs.doMock('./sum', { mock: true });

  const path = './sum';
  const mocked = await rs.importMock<typeof import('./sum')>(path);

  expect(rs.isMockFunction(mocked.sum)).toBe(true);
  expect(mocked.sum(1, 2)).toBeUndefined();
});
```

`rs.mock` is hoisted; `rs.doMock` takes effect when it executes, so call it before `rs.importMock`. Variable paths look up registered mocks. They do not discover additional modules or `__mocks__` files at runtime, even if those files exist on disk.

## rs.requireMock

- **Type:** `<T = Record<string, unknown>>(path: string) => T`

Loads the mocked exports of a CommonJS module synchronously. Use `rs.mockRequire` or `rs.doMockRequire` to register the mock with CommonJS resolution. For asynchronous loading, use [`rs.importMock`](#rsimportmock).

As with `rs.importMock`, a registered mock takes precedence over a matching `__mocks__` file. A string literal can load a manual mock without prior registration.

When given a variable path, `rs.requireMock` attempts to find a registered mock. It does not load additional modules or discover `__mocks__` files at runtime. Relative paths are resolved from the file containing the call. If no matching mock is available, `rs.requireMock` throws a module-not-found error synchronously.
...
test('loads the registered CommonJS mock', () => {
  const path = './math.cjs';
  const mockedMath = rs.requireMock(path);

  expect(mockedMath.sum(1, 2)).toBe(100);
});
```

To auto-mock the module, use `rs.mockRequire('./math.cjs', { mock: true })`. If using `rs.doMockRequire`, call it before `rs.requireMock` because it is not hoisted.
