...
When using Rspack, these re-exports may be further optimized: the export can be resolved directly from the source module (e.g. `react-router`), skipping the intermediate module (`react-router-dom`).
...
2. Disable the related bundler optimization in the test environment to prevent re-export flattening. Should be careful about this, disabling it may lead to errors related to runtime circular dependencies.
