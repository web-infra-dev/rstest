...
If the request has already been registered with `rs.mock` or `rs.doMock`, `rs.importMock` uses that registered factory first. Without a registered factory, a matching manual mock in `__mocks__` is still used, followed by the normal auto-mock fallback. The request may be a variable when it is resolved from a bundled test module.

If a runtime request cannot be resolved to a bundled module or a registered mock, `rs.importMock` rejects with a module-not-found error.
...
When the request is held in a variable, Rstest rewrites the call to its synchronous runtime helper as long as the caller is in a bundled test module. The module still needs to be represented in the bundle or mock registry.
