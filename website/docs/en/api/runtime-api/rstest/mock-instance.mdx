---
title: Mock instance
description: MockInstance is the type of all mock/spy instances, providing a rich set of APIs for controlling and inspecting mocks.
overviewHeaders: [2, 3]
---
...
```ts
const fn = rs.fn().mockImplementation(() => 1);
fn.mockReset();
...
{
  using spy = rs.spyOn(obj, 'foo').mockImplementation(() => 2);
...
<ApiMeta addedVersion="0.11.3" />

- **Type:** `(value: unknown) => MockInstance`

Makes the mock throw the specified value whenever it is called.

```ts
const fn = rs.fn().mockThrow(new Error('fail'));
...
Makes the mock throw the specified value on the next call only.

```ts
const fn = rs.fn(() => 'ok').mockThrowOnce(new Error('fail'));
expect(() => fn()).toThrow('fail');
expect(fn()).toBe('ok');
```
