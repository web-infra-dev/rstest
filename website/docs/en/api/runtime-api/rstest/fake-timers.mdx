...
`rs.useFakeTimers()` returns a `Disposable` that works with the `using` syntax to restore real timers automatically when the block exits.

The `_timers` binding is intentionally unused, as indicated by its leading underscore. A binding is required by the `using` syntax, so it cannot be omitted.
...
- **Alias:** `rstest.getRealTimers`
- **Type:** `() => { setTimeout: typeof globalThis.setTimeout; clearTimeout: typeof globalThis.clearTimeout; setImmediate?: typeof globalThis.setImmediate }`

Returns real timer functions captured by Rstest, even when fake timers are enabled.

```ts
const { setTimeout, clearTimeout } = rs.getRealTimers();

const timeout = setTimeout(() => {
  // This timer uses real time.
}, 1000);
