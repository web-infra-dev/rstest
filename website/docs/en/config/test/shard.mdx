---
description: Shards test files for parallel execution. count represents the total number of shards, and index represents the index of the current shard (1-based).
---
...
```bash
# Shard 1
npx rstest run --shard 1/3 --reporters=blob --coverage

# Shard 2
npx rstest run --shard 2/3 --reporters=blob --coverage

# Shard 3
npx rstest run --shard 3/3 --reporters=blob --coverage
```
