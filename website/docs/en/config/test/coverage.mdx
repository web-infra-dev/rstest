...
- switch to `coverage.provider: 'v8'` for Node or Chromium Browser Mode tests;
- avoid serializing Istanbul-instrumented functions.

In Chromium Browser Mode, the `v8` provider avoids injected counters as well. Firefox and WebKit Browser Mode tests should use `coverage.exclude` or avoid serializing instrumented functions.
...
The `v8` provider works in `node`, `jsdom`, and `happy-dom` test environments, and in Browser Mode when using Chromium with the Playwright provider. Chromium coverage is collected from the executed browser bundle and remapped through source maps, without instrumenting application code. Firefox and WebKit do not expose the required coverage API, so use the default `istanbul` provider for those browsers.
