...
Native browser V8 coverage currently profiles scripts in the page renderer target only. It does not collect code that runs exclusively in dedicated `Worker` or `SharedWorker` targets. Use `istanbul` when worker-only code must contribute coverage.
