...
An explicit per-call timeout takes precedence over this configuration. For Browser Mode `expect.element()` and `@rstest/playwright` locator/page assertions, it is still capped by the remaining active test or hook time when that context is available. For `expect.poll()`, an explicit timeout is used as provided and can still be ended by the outer test or hook timeout. When no explicit timeout is provided, Rstest uses the smaller of the configured timeout and the remaining active time. For example, a configured `5000ms` timeout with only `2000ms` remaining does not grant the assertion another five seconds.
