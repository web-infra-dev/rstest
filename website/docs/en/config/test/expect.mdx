...
An explicit per-call timeout takes precedence over this configuration, but it is still capped by the remaining active test or hook time when that context is available. For `expect.poll()` and Browser Mode `expect.element()`, Rstest uses the smaller of the requested or configured timeout and the remaining active time. For example, a configured `5000ms` timeout with only `2000ms` remaining does not grant the assertion another five seconds, and an explicit `5000ms` timeout is capped the same way. An explicit timeout does not extend the outer [`testTimeout`](/config/test/test-timeout) or hook timeout.
...
When using [`definePlaywrightConfig`](/guide/basic/e2e-testing#e2e-defaults), the helper defaults `expect.poll.timeout` to `5000ms`. An explicit `expect.poll.timeout` overrides that value for both polling and Playwright locator/page assertions. Without the helper, Node Mode defaults to `1000ms`.
