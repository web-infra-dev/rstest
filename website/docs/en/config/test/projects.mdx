...
Rstest supports nested project definitions within the rstest config of subprojects. This allows you to define more test projects in subprojects without having to define all projects in the root project.

This is especially useful when your subprojects need to support multiple test environments or multiple configurations.

### 示例

在 monorepo 中定义每个子项目为一个 project：
...
为 `packages/pkg-a` 定义 Node.js 和浏览器两个测试环境：
