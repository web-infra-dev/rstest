...
:::info `vmThreads` always isolates test files
This option has no effect when [`pool.type`](/config/test/pool#vmthreads) is `vmThreads`. Rstest reuses the worker thread but creates a fresh VM context, module graph, and test environment for every file. Setup files and worker-scoped fixtures therefore also remain file-scoped.
:::
