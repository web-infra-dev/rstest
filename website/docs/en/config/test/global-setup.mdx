...
:::note
Global setup runs in a separate context: module-scope variables defined in the setup file are not accessible from tests. However, `process.env` mutations made in `globalSetup` are snapshotted after setup completes and propagated to every test worker — setting environment variables here is a supported way to share values with tests.
:::
