...
If Rstest cannot build or load a prebundle, it falls back to the environment's native entry. The environment package is resolved from the project's dependency tree first, then from the Rstest workspace root.
