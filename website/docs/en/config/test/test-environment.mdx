...
The current automatic compatibility matrix covers jsdom 15–30 and happy-dom 20. Other major versions stay on the native path unless `prebundle: true` is set explicitly.
...
If Rstest cannot build or load a prebundle, it falls back to the environment's native entry. The environment package is resolved from the project's dependency tree first, then from the Rstest workspace root, and finally through `@rstest/core`'s native dependency resolution for backward compatibility.
