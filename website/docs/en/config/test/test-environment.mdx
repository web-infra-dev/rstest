...
`transformMode` controls whether a custom environment should use Node-like or browser-like transform and resolution defaults.

Custom environments use `transformMode: 'node'` by default unless you set it explicitly. Built-in environments infer the value automatically, so you usually only need this option when configuring a custom environment.
...
#### Migrating custom environments from Jest or Vitest \{#migrating-custom-environments}
