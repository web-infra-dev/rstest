...
When using a Node worker pool (`forks`, `threads`, or `vmThreads`), the options must be structured-cloneable because they are sent to a worker. Function-valued options such as `beforeParse` are not supported in these pools and are rejected before a test file is dispatched.
