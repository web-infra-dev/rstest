...
`transformMode` controls whether a custom environment should use Node-like or browser-like transform and resolution defaults. Built-in environments infer this value automatically, so you usually only need this option when configuring a custom environment.

Use `transformMode: 'node'` when the custom environment should use the same transform defaults as the built-in `node` environment. In this mode, Rstest keeps Node-oriented resolution defaults and externalizes third-party dependencies from `node_modules` by default.

Use `transformMode: 'browser'` when the custom environment should use the shared browser-like transform defaults of the built-in `jsdom` and `happy-dom` environments. In this mode, Rstest resolves browser-oriented package entrypoints by adding the `browser` condition when `resolve.conditionNames` is not customized, and bundles third-party dependencies by default so browser field replacements and bundled assets can take effect.
...
If you are migrating a custom environment from Jest or Vitest, see [Migrating Jest custom environments](/guide/migration/jest#custom-environments) or [Migrating Vitest custom environments](/guide/migration/vitest#custom-environments).
