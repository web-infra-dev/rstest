...
`target` does not create DOM APIs by itself and does not enable [browser mode](/config/test/browser). It only selects the build and resolution defaults. The environment implementation is still responsible for installing globals such as `window` and `document`.
