...
`onlyFailures` applies only to a plain, full run. Whenever the run is narrowed some other way, that selection wins and `onlyFailures` is ignored with a warning — a warning rather than an error, so `onlyFailures` set in a config file keeps working in these scenarios:
