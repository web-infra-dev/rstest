...
Each `vmThreads` worker also keeps a bounded cache of immutable bundle bytes, source maps, external source text, and V8 compilation data between files. After the first file, unchanged assets are referenced from that worker instead of being transferred again, and unchanged modules can skip part of the parse/compile work. These entries share one cache budget: at most 64 MiB or one quarter of the worker's `memoryLimit`, whichever is smaller. Rstest disables V8's unbounded isolate-wide compilation cache for these workers; the bounded cache supplies the reusable compilation data instead. The cache is cleared on rebuild or worker recycle. Evaluated module instances and setup execution state are not cached across VM contexts.

External JavaScript ES modules, CommonJS modules, and JSON modules execute inside the file VM. On Node.js 24.9 or later, CommonJS modules can synchronously `require()` ES modules that do not use top-level await; the complete dependency graph remains in the file VM. An async graph throws `ERR_REQUIRE_ASYNC_MODULE`, matching Node.js. Earlier Node.js versions reject `require()` of an ES module in `vmThreads`; use dynamic `import()` instead. Bundle external files with other extensions, such as TypeScript source or custom loaders, before running them in `vmThreads`.
