...
- You need strict process-level isolation for every test file. Use `forks`; `vmForks` reuses child processes and only creates a fresh VM context for each file.
