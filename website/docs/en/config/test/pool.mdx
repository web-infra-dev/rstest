...
Custom Node loaders and external TypeScript execution are outside this scope; compile or bundle those files first. The supported loading paths do not imply universal cross-realm value conversion or cancellation of arbitrary asynchronous work. Failures within a supported path are compatibility bugs, not additional implicit limitations.
