...
Across the Node builtin boundary, Rstest adapts plain objects, arrays, Promise fulfillment values, and Error rejection reasons to the current VM so common prototype and `instanceof Error` checks work as expected. This is not a general conversion of every value between realms: Node-native objects such as Buffers, streams, typed arrays, and native handles retain their native backing and prototypes. DOM objects created by jsdom and happy-dom likewise retain the prototype of their provider realm. Do not rely on constructor identity when passing values across realms.
