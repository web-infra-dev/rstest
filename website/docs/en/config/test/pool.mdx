...
External JavaScript ES modules, CommonJS modules, and JSON modules execute inside the file VM. CommonJS modules can `require()` other CommonJS or JSON modules, but `require()` of an ES module is rejected to avoid evaluating that dependency in the worker's host realm; use dynamic `import()` for that dependency instead. Bundle external files with other extensions, such as TypeScript source or custom loaders, before running them in `vmThreads`.
