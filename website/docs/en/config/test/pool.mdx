...
`vmForks` uses `child_process.fork` for process-level isolation and creates a fresh `vm.Context` for every test file. It has the same VM module loading and cross-realm behavior as `vmThreads`, while keeping process APIs such as `process.chdir()` available inside the worker. Worker recycling is handled by letting the child process exit.
...
Consider `vmForks` when the suite needs VM isolation but also relies on process-level APIs or separate child-process heaps. Consider `vmThreads` when avoiding child-process startup is more important and the suite is compatible with worker-thread limitations.
...
- You need strict process-level isolation. Use `forks` or `vmForks`; `vmThreads` deliberately shares the worker thread's process state.
