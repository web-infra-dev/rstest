...
`pool.memoryLimit` currently applies only to `vmThreads`. It is checked against the worker's V8 `heapUsed` after each file; a worker over the limit is recycled before another file is assigned to it. Passing it to `forks` or `threads` is currently ignored. Numbers in `(0, 1]` are treated as a fraction of system memory, larger numbers are bytes, and strings may use `%`, `MB`, `MiB`, `GB`, or `GiB`. When omitted, each VM worker receives an equal share of system memory.
