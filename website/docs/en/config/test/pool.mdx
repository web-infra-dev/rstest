...
The VM context does not isolate process-level state. `process.env`, `process.chdir`, signal handlers, native addons, and modules loaded through Node's native `require` still belong to the worker thread and can be shared by files. VM contexts also have cross-realm identity, so values created in one file may not pass `instanceof` checks against constructors from another file.
...
- You want `isolate: false` to affect Rstest fixture cleanup while retaining a fresh VM realm for every file.
