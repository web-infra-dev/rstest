...
For compatibility, the callback is typed with `RstestConfig`, but runtime validation still rejects fields that affect the global test plan or Rsbuild environment topology. For example, modifying `coverage`, `reporters`, `pool`, `isolate`, `projects`, `browser.enabled`, `plugins`, `extends`, or `output.distPath` will throw an error that tells you to configure those options in the Rstest config instead.
...
`modifyRstestConfig` is suitable for modifying existing project config such as `include`, `exclude`, `testEnvironment`, `resolve`, `source`, and `output.module`, but it does not support dynamically adding or removing Rstest projects, switching `browser.enabled`, modifying global options such as `coverage`, `reporters`, `pool`, and `isolate`, or changing Rsbuild environments and Rsbuild plugins.
