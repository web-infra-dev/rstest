...
`modifyRstestConfig` is suitable for modifying existing project config such as `include`, `exclude`, `testEnvironment`, `resolve`, and `source`, but it does not support dynamically adding or removing Rstest projects, switching `browser.enabled`, Rsbuild environments, or Rsbuild plugins.
