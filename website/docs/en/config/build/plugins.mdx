...
When an Rsbuild plugin runs inside Rstest, it can get the Rstest exposed API through `api.useExposed('rstest')` from the Rsbuild [Plugin API](https://rsbuild.rs/plugins/dev/core) and call `modifyRstestConfig` to adjust the Rstest config for the current project.
