...
Rstest resolves file-level environment comments before Rsbuild initializes the environments for a test run, so environment-scoped Rsbuild plugins can be applied to files that use comments such as `@vitest-environment jsdom`. Because `modifyRstestConfig` runs during Rsbuild config resolution, it must not add or reveal test files that require new file-level environment comment groups. If a plugin needs those files to run in a different environment, configure `testEnvironment` or separate `projects` before Rsbuild initialization instead.
