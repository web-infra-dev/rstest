...
When configuring Rstest directly, it does not use other `compilerOptions` to configure code transformation. Configure the output target with [`tools.swc`](/config/build/tools#toolsswc), and configure decorator syntax explicitly with [`source.decorators`](#sourcedecorators). An adapter may map or infer additional build options; refer to the documentation for the [adapter you use](/guide/integration/adapters). For example, a directly configured project that uses a legacy decorator transform and ES2020 output can use:
