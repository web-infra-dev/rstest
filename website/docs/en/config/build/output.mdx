...
- `'auto'` (default): Bundle dependencies in browser-like test environments (jsdom, happy-dom, etc.), and externalize them in the `node` environment.
...
:::warning
This option only applies to non-browser mode. In [browser mode](/guide/browser/index), all dependencies are always bundled and this option has no effect.
:::
