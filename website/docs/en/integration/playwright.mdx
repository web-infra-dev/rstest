...
| Playwright locator/page assertion timeout | `5000ms`                                                            |
| Rstest `expect.poll.timeout`              | `definePlaywrightConfig`'s `expect.timeout`, or `5000ms` when unset |
...
Here, Playwright locator/page assertions use `10_000ms`, while `expect.poll()` uses `2000ms`. Per-call timeout options still take precedence for both APIs.
...
Set a shared timeout for all Playwright locator and page assertions with `definePlaywrightConfig`:
...
Timeout precedence is: the matcher's `{ timeout }`, then `definePlaywrightConfig`'s `expect.timeout`, then `5000ms`. This also applies to `.not` and `expect.soft`. The outer test or hook timeout can still end the assertion sooner.

The helper also uses `expect.timeout` as the default for Rstest's `expect.poll()`. You can override polling separately with top-level `expect.poll.timeout`. Changes to that top-level polling setting do not change the Playwright locator/page assertion timeout.

Rstest's [`expect.poll.timeout`](/config/test/expect#expectpolltimeout) controls `expect.poll()` and Browser Mode `expect.element()`; it does not affect these Playwright assertions. `page.setDefaultTimeout()` controls Playwright operations and does not change this assertion timeout. Rstest does not read `playwright.config.ts`.
