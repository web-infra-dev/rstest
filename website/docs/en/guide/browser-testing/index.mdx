...
Module mocking (the [rs.mock family](/api/runtime-api/rstest/mock-modules)) and [includeSource](/config/test/include-source) in-source testing behave the same as in Node mode. `rs.mockRequire` also works in browser tests, but it exists for CommonJS interop — prefer `rs.mock` / `rs.doMock` when writing browser tests.
