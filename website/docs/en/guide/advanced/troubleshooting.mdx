...
On Node.js versions that support running `.ts` files directly, `require('./plugin.ts')` can still fail because Node.js classifies `plugin.ts` as ESM under the package `type: module` scope:
