...
- [@rstest/coverage-v8](https://github.com/web-infra-dev/rstest/tree/main/packages/coverage-v8): V8-based provider for Node mode and Chromium Browser Mode.
...
Each shard writes `.rstest-reports/blob-{index}-{count}.json`. The merge job collects those files into one `.rstest-reports/` directory, then `rstest merge-reports` runs the configured reporters on the unified result and merges coverage data. See [sharding](/guide/basic/cli#sharding-tests) and [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) for details.
