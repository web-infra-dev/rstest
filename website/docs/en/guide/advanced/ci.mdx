...
Each shard writes `.rstest-reports/blob-{index}-{count}.json` with its test results and collected coverage. Shards do not generate coverage reports or check thresholds. The merge job collects those files into one `.rstest-reports/` directory, then `rstest merge-reports` adds untested files, runs the configured reporters, and checks coverage thresholds against the unified result. See [sharding](/guide/basic/cli#sharding-tests) and [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) for details.
