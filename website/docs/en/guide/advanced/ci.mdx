...
Configure [@rstest/playwright](/integration/playwright) launch defaults in `rstest.config.ts`:
...
With this fixture, a standard GitHub-hosted workflow only needs to install npm dependencies and run Rstest; do not add a `playwright install` step. See the [@rstest/playwright guide](/integration/playwright) for fixture usage and trace artifacts.
