...
Configure [@rstest/playwright](/guide/basic/e2e-testing) launch defaults in `rstest.config.ts`:
...
With this fixture, a standard GitHub-hosted workflow only needs to install npm dependencies and run Rstest; do not add a `playwright install` step. See the [@rstest/playwright guide](/guide/basic/e2e-testing) for fixture usage and trace artifacts.
