...
Run rstest with the `--profile` flag to get a ready-to-run samply command with the rstest entry path resolved for you:

```bash
rstest --profile
```

If samply is installed, rstest prints the exact command to copy and run. If samply is not installed, rstest prints install instructions (`cargo install samply`, or `brew install samply` on macOS) along with the command to run after installing.

We deliberately do not spawn samply directly — rstest only resolves and prints the command, so signal handling, exit codes, and shell history all stay with samply itself.

### Manual command

You can also run samply directly without going through `--profile`:
