...
In Browser Mode the plugin does not turn on the `federation` option for you. Set `federation: true` yourself when the project uses `globalSetup`: the setup file runs in a Node worker with the same federation build, and without the switch the federation runtime throws while the file loads, so the remote server never starts.
