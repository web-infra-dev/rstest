...
## Midscene for AI-driven tests

Selectors break when the UI changes. [Midscene](/guide/advanced/midscene) builds on `@rstest/playwright` and lets you describe interactions and assertions in natural language instead, with a visual HTML report that replays each AI step with screenshots:
...
Every fixture and option on this page stays available, so you can mix `agent` calls with plain Playwright APIs in the same test.
