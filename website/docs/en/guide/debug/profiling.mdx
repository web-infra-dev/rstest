...
If you are using a Coding Agent that supports Skills, install the [rstest-debugging](https://github.com/rstackjs/agent-skills#rstest-debugging) skill to debug Rstest issues systematically, including performance regressions. Use this skill when Rstest is slower than expected, slower than Jest or Vitest, or when you need to determine whether the bottleneck is in build startup or test execution before trying config or code changes.

```bash
npx skills add rstackjs/agent-skills --skill rstest-debugging
```

After installation, let the Coding Agent help you investigate performance issues.
...
To measure that split instead of inferring it, run the suite once with [`--trace`](#using-trace).
...
**2. Break the run down by phase, suite, and case**

Rerun the same tests with [`--trace`](#using-trace). Its summary ranks the slowest phases, files, and test cases, which usually tells you whether the cost sits in build-related phases, in setup files, or in the test bodies.

**3. Inspect detailed time distribution after narrowing the scope**
...
## Using --trace \{#using-trace}

`--trace` breaks a run into per-phase, per-suite, and per-case slices. Use it when the reporter's single `Duration` number is not enough to tell where the time is spent.

```bash
npx rstest run --trace
```

The flag is CLI-only and opt-in. Each run records per-suite and per-case slices annotated with status and retry count, a heap counter sampled at each phase boundary, and one slice per phase of every test file:

- `prepare`: set up the worker runtime, the global API, and the coverage provider when coverage is enabled. It is entered twice per test file, before and after `envSetup`.
- `envSetup`: install the test environment. Close to zero for `node`, and when a non-isolated worker is already pinned to the environment.
- `load`: receive the built assets of the test entry. The host-side cost of producing them is reported as a host span instead.
- `setupFiles`: evaluate the configured setup files.
- `collect`: evaluate the top-level code of the test file. `describe` bodies are deferred and run in `tests`.
- `tests`: build the suite and case tree, then run the cases and their hooks.
- `coverage`: collect coverage data from the provider. Absent when coverage is disabled, or when the file bailed or threw before this point.
- `teardown`: clean up the coverage provider and the worker state, plus the test environment when `isolate` is enabled.

Browser Mode records only `prepare` and `tests`, measured on the host from the messages the page sends.

Rstest writes two files to `<rootPath>/.rstest/`, and replaces them on every watch rerun:

- `trace-<timestamp>.summary.md`: a markdown summary ranking the slowest phases, files, and test cases. The same content is printed to the terminal.
- `trace-<timestamp>.json`: a Perfetto-compatible trace for the visual UI.

Start with the summary. Open the trace when you also need to see how work overlapped across workers.
...
In a TTY, Rstest starts a local helper server so that the printed link opens the trace directly in the Perfetto UI. The run then stays alive until you press Ctrl+C to stop that server. In CI, the helper server is not started. Upload the trace file as an artifact and drag it into [ui.perfetto.dev](https://ui.perfetto.dev) instead.

:::tip
To investigate a slowdown with a Coding Agent, hand it both files: the summary for the ranking, the trace for the full event data. They pair with the [rstest-debugging](#using-agent-skills) skill.
:::
...
Choose a profiler based on what you need to analyze. Check whether [`--trace`](#using-trace) already answers the question first, since it requires no external tool.
