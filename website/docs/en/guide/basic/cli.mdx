...
| `--teardownTimeout <value>`         | Time to wait for the process to exit after the run finishes, in milliseconds. 0 exits immediately. See [teardownTimeout](/config/test/teardown-timeout).                                              |
...
| `--pool.memoryLimit <limit>`        | Memory recycling threshold for `forks` with `isolate: false` (RSS) and VM pools (V8 heap), see [pool](/config/test/pool#poolmemorylimit)                                                              |
