...
| `--pool.memoryLimit <limit>`        | Memory limit for `vmForks` and `vmThreads` workers before recycling; currently ignored by `forks` and `threads`, see [pool](/config/test/pool)                                                        |
