...
- Merge the coverage collected by all shards (if coverage is enabled)
- Apply `coverage.include` once to add files that no shard tested
- Generate coverage reports and check thresholds against the unified result

With coverage providers that support deferred finalization, shard runs using the blob reporter deliberately skip coverage report generation and threshold checks. Older providers retain their existing per-shard finalization behavior. Enable coverage for `merge-reports` through your configuration or the `--coverage` flag so the merge job can finalize coverage. The merge command accepts the same `--coverage.*` options as `rstest run`. Coverage options passed only to a shard are not stored in its blob, so define finalization options such as `coverage.include`, `coverage.reporters`, and `coverage.reportsDirectory` in the configuration file or repeat them on `merge-reports`.
...
| `--coverage.allowExternal`          | Collect coverage for files outside the project root, see [coverage.allowExternal](/config/test/coverage#allowexternal)                                                                                |
| `--coverage.changed [commit]`       | Collect coverage only for changed files in the current Git repository, optionally since a commit                                                                                                      |
| `--coverage.clean`                  | Clean the coverage directory before running tests, see [coverage.clean](/config/test/coverage#clean)                                                                                                  |
| `--coverage.enabled`                | Enable code coverage collection, see [coverage.enabled](/config/test/coverage#enabled)                                                                                                                |
| `--coverage.exclude <pattern>`      | Exclude files from coverage collection, see [coverage.exclude](/config/test/coverage#exclude)                                                                                                         |
| `--coverage.include <pattern>`      | Include files for coverage collection, see [coverage.include](/config/test/coverage#include)                                                                                                          |
| `--coverage.provider <provider>`    | Choose the coverage provider (`istanbul` \| `v8`), see [coverage.provider](/config/test/coverage#provider)                                                                                            |
| `--coverage.reporters <reporter>`   | Specify coverage reporter(s), see [coverage.reporters](/config/test/coverage#reporters)                                                                                                               |
| `--coverage.reportOnFailure`        | Generate coverage reports even when tests fail, see [coverage.reportOnFailure](/config/test/coverage#reportonfailure)                                                                                 |
| `--coverage.reportsDirectory <dir>` | Directory to store coverage reports, see [coverage.reportsDirectory](/config/test/coverage#reportsdirectory)                                                                                          |
| `--dev.writeToDisk`                 | Write test temporary files to disk, see [dev.writeToDisk](/config/build/dev#devwritetodisk)                                                                                                           |
...
| `--includeTaskLocation`             | Collect test and suite locations, see [includeTaskLocation](/config/test/include-task-location)                                                                                                       |
...
| `-f, --onlyFailures`                | Run only the test files that failed in the previous run, see [onlyFailures](/config/test/only-failures)                                                                                               |
| `--output.cleanDistPath`            | Clean test temporary files before the test starts, see [output.cleanDistPath](/config/build/output#outputcleandistpath)                                                                               |
| `--output.emitAssets`               | Emit imported static assets, see [output.emitAssets](/config/build/output#outputemitassets)                                                                                                           |
| `--output.module`                   | Output JavaScript files in ES module format, see [output.module](/config/build/output#outputmodule)                                                                                                   |
...
| `--reporters, --reporter <name>`    | Specify the test reporter(s), see [reporters](/config/test/reporters)                                                                                                                                 |
...
| `--source.tsconfigPath <path>`      | Path to the tsconfig.json file, see [source.tsconfigPath](/config/build/source#sourcetsconfigpath)                                                                                                    |
...
| `--trace`                           | Dump a Perfetto-compatible performance trace JSON file, plus a ranked markdown timing summary printed to the terminal and written next to it, see [Using --trace](/guide/debug/profiling#using-trace) |
...
| `--reporters <name>`       | Specify which reporter(s) run on merged results, see [reporters](/config/test/reporters)                                                                              |
