...
Shard runs using the blob reporter deliberately skip coverage report generation and threshold checks. Enable coverage for `merge-reports` through your configuration or the `--coverage` flag so the merge job can finalize coverage.
