...
`--changed` cannot be combined with positional filters or `--related` / `--findRelatedTests`, because it already provides the source-file filter from Git state.
...
The shortcuts also work in Browser Mode watch sessions: `f`, `a`, `u`, and `q` drive the browser test reruns, while `t` and `p` are not supported in browser watch yet (the help menu greys them out). In mixed Node.js + browser watch runs, a single shortcut applies to both sides — for example, `a` reruns all Node.js and browser tests, and `q` shuts both down.
