---
description: This E2E testing guide covers how to use @rstest/playwright to test complete pages and apps from Rstest tests running in Node.js workers.
---
...
The type-only exports include `APIRequestContext`, `APIResponse`, `Browser`, `BrowserContext`, `BrowserContextOptions`, `BrowserType`, `Frame`, `LaunchOptions`, `Locator`, `Page`, `Request`, `Response`, and `Route`.

`@rstest/playwright` is an Rstest integration layer, not a replacement for the `playwright` package. It intentionally does not re-export runtime APIs such as `chromium`, `firefox`, `webkit`, or `request`. Import those APIs directly from `playwright` when you need them:

```ts
import { chromium, request } from 'playwright';
```

For normal E2E tests, use the `browser`, `context`, `page`, and `request` fixtures instead. Rstest creates and cleans up those resources according to the fixture lifecycle.
...
Playwright locator and page assertions read Rstest's [`expect.poll.timeout`](/config/test/expect#expectpolltimeout). Configure it once to control both these assertions and `expect.poll()`:
