...
1. **CLI**: `--reporters=<name>` (can be used multiple times; `--reporter` is also accepted as an alias)
...
<Tabs defaultValue='rstest.config.ts'>
  <Tab label="CLI">
```bash
npx rstest --reporters=github-actions
```
  </Tab>
  <Tab label="rstest.config.ts">
```ts
...
| `summary`     | `boolean \| { maxCharsPerField?: number }` | `true`  | Controls Markdown summary output and per-field length.    |

<ApiMeta addedVersion="0.11.6" />

Use the object form of `summary` to customize the maximum characters included in each failure message and diff, and each unhandled error message and stack. `maxCharsPerField` defaults to `400`. GitHub limits the whole Summary generated by one step to [1 MiB](https://docs.github.com/en/actions/writing-workflows/choosing-what-your-workflow-does/workflow-commands-for-github-actions#step-isolation-and-limits), so keep the configured value bounded. Set `summary` to `false` to disable the Markdown summary.
...
The blob reporter serializes test results into JSON files for later merging. This is designed for use with [test sharding](/guide/basic/cli#sharding-tests) in CI environments, where tests are split across multiple machines and results need to be combined afterward. Like sharding, it is not supported in watch mode.
...
Blob files are saved to `.rstest-reports/` by default. When coverage is enabled, each blob stores only the coverage collected during that run. With coverage providers that support deferred finalization, runs using the blob reporter do not generate coverage reports, add untested files from `coverage.include`, or check coverage thresholds. Runs without the blob reporter finalize coverage normally, including sharded runs. Older providers retain their existing per-run finalization behavior. After all blob-producing runs complete, use [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) with coverage enabled to perform those steps once against the unified result. Coverage options passed only on a blob-producing run are not stored in the blob; keep finalization options in the configuration file or repeat them on `merge-reports`.

See [splitting tests with shards](/guide/advanced/ci#split-tests-with-shards) for a complete CI workflow example.
