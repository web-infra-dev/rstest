...
Blob files are saved to `.rstest-reports/` by default. When coverage is enabled, each blob stores only the coverage collected while that shard ran. With coverage providers that support deferred finalization, shard runs do not generate coverage reports, add untested files from `coverage.include`, or check coverage thresholds. Older providers retain their existing per-shard finalization behavior. After all shards complete, use [`rstest merge-reports`](/guide/basic/cli#rstest-merge-reports) with coverage enabled to perform those steps once against the unified result. Coverage options passed only on a shard command are not stored in the blob; keep finalization options in the configuration file or repeat them on `merge-reports`.
