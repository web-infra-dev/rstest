---
description: This chapter introduces how to configure Rstest and its underlying Rsbuild and Rspack.
---
...
If your tests use filenames such as `*.test.ts` or `*.spec.ts`, scope the preset to those files:
...
The recommended preset does not declare a `files` pattern. Adding one keeps Rstest-specific rules focused on test files and leaves production files to your other Rslint configuration items.
...
See [Modify Rstest config in Rsbuild plugins](/config/build/plugins#modify-rstest-config) for usage, type definitions, and limitations.
...
In large projects, you often already have existing build/toolchain configurations (aliases, global variables, plugins, etc.). Through [adapters](/guide/integration/adapters) and the [extends](/config/test/extends) option, Rstest can transform and integrate these external configurations into your test setup, helping you avoid duplicate maintenance and keep configurations consistent.
