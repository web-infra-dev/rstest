...
Use `istanbul` when you want SWC-based instrumentation with stable Istanbul semantics, or `v8` when coverage memory usage is the main bottleneck. In Chromium Browser Mode, `v8` collects native coverage without instrumenting application code; Firefox and WebKit use `istanbul`.
