...
Rstest is built on top of Rspack and Rsbuild, and can directly **reuse your existing build configuration**—including module resolution, code transformation via SWC, and plugin capabilities. This eliminates the need to maintain separate transform or resolver configurations for testing, allowing developers to get started with minimal setup. At the same time, even if you are not using Rspack, Rstest can be adopted without migrating your existing build system.
