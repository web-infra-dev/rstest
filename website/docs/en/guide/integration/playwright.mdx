...
When `PWDEBUG=1` is enabled, `serve` keeps the server alive by default so the opened page remains available for inspection. Set `keepAliveOnDebug: false` if you want the server to close even in debug mode.
