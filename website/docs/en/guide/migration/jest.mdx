...
| `jest --forceExit`                       | `rstest --teardownTimeout 0`                      | Rstest exits immediately after teardown functions and reporters finish.                                           |
| `jest --openHandlesTimeout=5000`         | `rstest --teardownTimeout 5000`                   | Rstest force exits after the timeout; Jest only prints a warning.                                                 |
...
| `openHandlesTimeout`         | [`teardownTimeout`](/config/test/teardown-timeout)                                                                     | Rstest force exits after the timeout; Jest only prints a warning.                                                                                                                                                                                                                                                                                                                                                                       |
| `forceExit`                  | [`teardownTimeout: 0`](/config/test/teardown-timeout)                                                                  | Exit immediately after tests, global teardown, and reporters finish.                                                                                                                                                                                                                                                                                                                                                                    |
| `detectOpenHandles`          | No exact equivalent; consider [`detectAsyncLeaks`](/config/test/detect-async-leaks) for test-file async leak debugging | Not a 1:1 mapping. `detectAsyncLeaks` fails test files with active async resources, but these leaks do not block process exit. Use [`teardownTimeout`](/config/test/teardown-timeout) to stop a run from hanging because of handles left open by reporters, plugins, or config.                                                                                                                                                         |
...
When migrating a Jest virtual mock, first [declare the module and configure `resolve.alias`](/api/runtime-api/rstest/mock-modules#mock-virtual-modules). This setup works in both Node and Browser mode. Then remove Jest's third argument:
