...
For a quick trial, you can temporarily expose Rstest's utilities under Jest's
global name from a [`setupFiles`](/config/test/setup-files) module:

```ts title='rstest.setup.ts'
import { rstest } from '@rstest/core';

Object.assign(globalThis, { jest: rstest });
```

Existing global calls such as `jest.fn()`, `jest.mock()`, and
`jest.requireActual()` then use Rstest. This alias is intended to reduce the
initial migration diff; prefer `rs` in new code.
