...
| `globalSetup`                | [`globalSetup`](/config/test/global-setup)                                                     | Rstest calls setup with no arguments — if your Jest setup reads `(globalConfig, projectConfig)`, rewrite without them.                                                                                                                                                                                                                                                                                                                  |
...
#### Don't shim — rewrite call sites

When migrating tests in bulk, it can be tempting to alias the old identifier so the diff stays small. All three forms below leave the migration incomplete and should be avoided:

```ts
// Avoid: direct global aliasing
globalThis.jest = rstest;

// Avoid: local rebinding
const jest = rstest;

// Avoid: aliased named import
import { rstest as jest } from '@rstest/core';
```

The test body still reads `jest.fn()` after these, so a reviewer cannot tell the migration happened, IDE rename refactors stop following the new identifier, and any future `@rstest/core` deprecation warning will target an identifier the test never actually uses. The correct form is the literal rewrite shown above: every `jest.<api>` call site becomes `rstest.<api>`.

#### Preserve test names during replace

`jest.` can also appear inside `describe` / `it` / `test` name strings (for example `it('jest.useFakeTimers advances time', ...)`). A blind find-and-replace that matches the bare identifier will rewrite these strings too, mutating test names that should be preserved.

Constrain the replacement to call sites only. A regex like `\bjest\.(fn|mock|spyOn|useFakeTimers|isolateModules|requireActual|requireMock|clearAllMocks|resetAllMocks|restoreAllMocks)\s*\(` matches just calls. After replacement, grep for `describe\(|it\(|test\(` and confirm no test name string changed.
...
- Separator-only key renames are formatting changes, not behavioural changes.
- Key-order churn without any body difference is also formatting-only.
- Body content changes under the same key are the only signal of a real behaviour change.
