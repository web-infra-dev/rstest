...
Vite reads selected `tsconfig.json` compiler options when transforming TypeScript. Rstest automatically uses `compilerOptions.paths` for module resolution, but does not use `target`, `experimentalDecorators`, or other compiler options to configure its transform. Move those settings to `tools.swc` and `source.decorators`; see [source.tsconfigPath](/config/build/source#sourcetsconfigpath). If your Vitest config enables Vite's TypeScript paths support or uses a `vite-tsconfig-paths` plugin, remove that Vite-specific setup after confirming the Rstest project uses the intended `tsconfig.json`.
