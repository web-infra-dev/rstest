...
| `vmMemoryLimit`               | [`pool.vmMemoryLimit`](/config/test/pool)                                                      | Applies when migrating Vitest `vmThreads` to Rstest `vmThreads`. Both recycle a VM worker after a file exceeds the threshold; Rstest also uses the value to bound its per-worker immutable asset and compilation cache.                                                                                                                                                                                                                                                                                                                                                      |
