...
| ----------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment`                 | [`testEnvironment`](/config/test/test-environment)                                             | Fold `test.environmentOptions` into the object form: `testEnvironment: { name: 'jsdom', options: { ... } }`. Rstest supports custom environment packages and JavaScript file paths via a Rstest environment object. Package resolution also tries `rstest-environment-*`. For custom environment migration details, see [Migrating Vitest custom environments](#custom-environments). File-level `@vitest-environment` comments are recognized for built-in environments.                                                                                                    |
...
| `poolOptions.forks.maxForks`  | [`pool.maxWorkers`](/config/test/pool)                                                         | Rstest flattens `poolOptions` into top-level `pool`: `poolOptions.forks.maxForks` → `pool.maxWorkers`, `execArgv` → `pool.execArgv`. `minForks` has no equivalent — Rstest does not expose a `minWorkers` knob. Only the `forks` pool is supported — `pool: 'threads' \| 'vmThreads' \| 'vmForks'` settings are dropped (behavior reverts to forks). Vitest 4's top-level `test.maxWorkers` maps to `pool.maxWorkers` (`test.minWorkers` is dropped).                                                                                                                        |
...
When migrating a Vitest custom environment, the object shape is closer, but `viteEnvironment` must be moved into Rstest config as `target`:

```diff title="Vitest to Rstest custom environment"
  export default {
    name: 'custom-client',
...
```diff title="Vitest config to Rstest config"
- import { defineConfig } from 'vitest/config';
+ import { defineConfig } from '@rstest/core';

  export default defineConfig({
-   test: {
-     environment: './custom-client-environment.mjs',
-     environmentOptions: {
...
Map `viteEnvironment: 'ssr'` to `target: 'node'` or omit `target`, and map `viteEnvironment: 'client'` to `target: 'web'`. For `vitest-environment-foo` packages, use the full package name or publish/rename them to `rstest-environment-foo`.

For more custom environment API details, see [`testEnvironment`](/config/test/test-environment#custom-environments).
