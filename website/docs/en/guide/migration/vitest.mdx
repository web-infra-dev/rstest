...
When migrating a Vitest custom environment, the object shape is closer, but `viteEnvironment` must be moved into Rstest config as `transformMode`:
...
Map `viteEnvironment: 'ssr'` to `transformMode: 'node'` or omit `transformMode`, and map `viteEnvironment: 'client'` to `transformMode: 'browser'`. For `vitest-environment-foo` packages, use the full package name or publish/rename them to `rstest-environment-foo`.
