...
For suites with many test files, you can use [pool.memoryLimit](/config/test/pool#poolmemorylimit) to limit the memory usage of a single worker. Once the threshold is exceeded, Rstest automatically replaces it with a new worker.
...
- **Task metadata.** Tests, suites, and files can carry custom metadata, which makes it easy to group results by owner, module, or any other dimension. See [Metadata](/guide/advanced/metadata).
