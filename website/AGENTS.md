...
- Keep `**Type:**` blocks structurally identical across both locales; `pnpm lint` enforces this parity, and only comments may differ.
- When documenting a newly added API or config option, add an `ApiMeta` version marker near that section. Import it with `import { ApiMeta } from '@components/ApiMeta';` and render it as `<ApiMeta addedVersion="x.y.z" />`. Use the intended release version when specified, including minor releases such as `0.12.0`; the current package version does not require new features to ship in the next patch. If no target release is specified, default to the owning package's current version with its patch segment incremented by 1 (e.g. `0.10.6` → `0.10.7`). Keep the English and Chinese markers aligned.
