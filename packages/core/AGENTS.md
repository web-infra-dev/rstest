...
- Watch is one core-owned loop (`src/core/watch/watchSession.ts`): an executor signals `TestExecutor.onInvalidate` with its scope already resolved, and core answers with reset → `runCycle` → `finalizeRunCycle` → next trace buffer → ready banner. Cycles queue, so two never interleave on the shared `stateManager`; a burst folds into the queued cycle only among same-kind triggers, and a fold unions file lists and nothing else — one trigger kind's state (e.g. `u`'s `updateSnapshot`) must never reach files another trigger chose. A trigger that resolves to no work must not signal.
...
- In watch the browser executor loads early — loading validates its config and can fail on a version mismatch, and the shared teardown and stdin owner close over it — but the **launch** (the first browser cycle) waits until node run resources are up, so a node dependency failure never leaves a browser mid-launch. `ensureRunResources` stays a required member of the node executor's own type, not an optional member of the shared seam `@rstest/browser` is version-locked to.
- An empty browser watch launch keeps the CLI alive through its session and prints the ready banner so added test files can run, without raising the exit code; browser globalSetup runs even with zero entries because the session will run files added later, while non-watch keeps the entry gate.
- Rerun-cycle executor rejections finalize an error cycle; first-cycle executor rejections and `globalSetup` failures reject startup in node, browser, and mixed watch alike. Compile callbacks deliver initial rejection through the driver's first-cycle Promise rather than throw into the compiler's fire-and-forget failure hook. Fatal bundler outcomes finalize and close the session through the driver; ordinary test compile errors keep watching. Mixed watch awaits browser startup rather than booting it in the background.
- Core is the single stdin/CLI-shortcuts owner for every watch shape (mirrored in `packages/browser/AGENTS.md`); `t`/`p` are node-only (the browser rerun pipeline takes no filter input) and render greyed hints otherwise. Rerun keys arm once every executor of the run has a _settled_ first cycle — settled, not succeeded — behind one gate that the prompt-opening `t`/`p` also check before asking for input. Startup failures close the session and release its shortcuts.
- Node watch planning builds its compiler environments and entry graph without startup file filters. Startup filters and interactive `p` selections narrow executor cycles only; the latter stays on the node watch target and must never be written back to the shared `InternalContext`, which the browser side also consumes. `a` clears that node selection and reuses the existing compiler graph.
- `src/core/environment/isBrowserProject.ts` stays outside `src/core/browser/` on purpose: it is the shared routing predicate every node-path module reads, not browser-mode implementation.
- Browser projects run their own pre-cycle `globalSetup` stage (`src/core/browser/globalSetupStage.ts`), distinct from the node `src/core/execution/globalSetup.ts` — don't merge the two.
