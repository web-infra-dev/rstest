...
- Base64 JavaScript `data:` URL setup entries are materialized as absolute virtual modules before entry assembly; setup state, cache control, and `VirtualModulesPlugin` must use that same materialized path so setup code reruns for every file under `isolate: false`.
