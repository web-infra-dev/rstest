...
- Reporter-internal per-file state (buffered console logs) tracks the `updateReporterResultState` snapshot: key it through `reporterFileKey` (project + path, never a path alone), reset that key on `onTestFileStart` so a watch rerun replaces instead of appending, and prune it against the `onTestRunEnd` result set so deleted files leave both together. The md report is one document at one scope for the same reason — its Failures section reads the session snapshot and must not be narrowed by `filterRerunTestPaths`, which would desync it from the summary counts.
