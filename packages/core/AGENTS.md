...
- A blob stores collected coverage before report-stage filtering or untested-file backfill when the provider advertises deferred finalization support. Providers without that capability must still finalize before blob serialization for cross-version compatibility; capable providers defer reports and thresholds to `merge-reports`, which owns finalization against the unified map.
