...
- Reporter-internal per-file state (buffered console logs) tracks the `updateReporterResultState` snapshot: key it through `reporterFileKey` (project + path, never a path alone) and reset that key on `onTestFileStart` so a watch rerun replaces instead of appending. Prune it against the result set at `onTestRunEnd`, but at path granularity (`reportedTestPaths`) — the snapshot itself is keyed by path, so only one project's result survives a file two projects share, and pruning at the finer buffer identity would discard the other project's logs. The md report is one document at one scope for the same reason — its Failures section reads the session snapshot and must not be narrowed by `filterRerunTestPaths`, which would desync it from the summary counts.
