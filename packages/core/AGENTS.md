...
- Under `isolate: false`, cache control registers test entries by canonical source path and the worker invalidates only the entry currently being dispatched. Clearing every discovered entry before every file breaks once-per-worker dependency state; registering logical paths misses symlinked entries after Rspack resolves them to real resources.
