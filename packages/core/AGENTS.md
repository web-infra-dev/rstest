...
- Reporter-internal per-file state (buffered console logs) obeys the same latest-per-file semantics as the `updateReporterResultState` snapshot: key it by test path and reset that key on `onTestFileStart`, so a watch rerun replaces instead of appending. The md report is one document at one scope for the same reason — its Failures section reads the session snapshot and must not be narrowed by `filterRerunTestPaths`, which would desync it from the summary counts.
