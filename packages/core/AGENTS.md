...
- `BlobData` is a same-version wire format between `BlobReporter` and `mergeReports`, and a `version` mismatch is rejected outright — never a partial merge. The merge replays the reporter lifecycle from `BlobFileData.events`, where each event stores the payload the reporter received verbatim and replay is pure playback — never reconstruct a payload from other blob data. Recording completeness is compile-guarded: `BLOB_TRACK_MATCHES_RUNNER_EVENTS` couples the track to the sink's lifecycle events, and `replayTestFile`'s switch is exhaustiveness-checked, so a new runner event or track variant fails the build until both sides handle it. A track lives for one run cycle: `onTestRunStart` marks existing tracks stale and a file's first event of the cycle replaces its track, which is what keeps watch-rerun blobs from replaying two runs against one result; a track still stale at run end with no surviving result — its file was deleted mid-watch, or never produced a result and was never rerun — is dropped from the blob. Tracks replay one file at a time — cross-file interleaving from parallel workers is deliberately not preserved (no such order can exist across shard blobs), so merged output reads like a `maxWorkers: 1` run.
