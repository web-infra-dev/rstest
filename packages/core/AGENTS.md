...
- Browser projects' `globalSetup` runs once per watch session, before the initial browser cycle and before the node dev server starts: its environment change-set is stored on the context and composed into both node workers and the browser launch, so a node cycle dispatched first would miss it. The node dependency check runs ahead of the stage in turn, so a node project that cannot run rejects the session before user setup runs. Reruns reuse the context-local change-set, and teardown drains with the watch teardown; the en/zh `global-setup` docs state all three. A failed setup is a cycle outcome, not a startup rejection: the launch is deferred, every trigger before it re-runs the stage (the stage itself skips projects that already succeeded, so their env has to accumulate across attempts), and the setup files get a watcher of their own because nothing else watches the browser side before the host exists.
...
- Rerun-cycle executor rejections finalize an error cycle; first-cycle executor rejections reject startup in node, browser, and mixed watch alike. Mixed watch awaits browser startup rather than booting it in the background.
