...
- JavaScript `data:` URL setup entries are materialized as absolute virtual modules before entry assembly; setup state, cache control, `VirtualModulesPlugin`, and coverage excludes must use that same materialized path. Virtual setup code executes but is intentionally not collected as coverage.
