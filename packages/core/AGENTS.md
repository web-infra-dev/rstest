...
- `src/core/runTests.ts` stays a coarse orchestrator — split projects → planner resolve → construct executors → drive them → finalize. Planning is core-owned and lives in `src/core/planner.ts`, never inside an executor: resolving the planner is the run's init barrier (the node `modifyRstestConfig` hooks fire and the plan is read while it is built), so no executor can be constructed against a plan that is still moving, and "which executors does this run need" is answerable before any of them exists. Browser-mode detail lives under `src/core/browser/`, behind the `BrowserHostModule` load boundary in `src/core/browser/loader.ts`, never inline in the orchestrator.
- One orchestrator branch breaks executor isomorphism, deliberately and commented at the call site: the browser-only fast path, because resolving the planner boots a node Rsbuild instance that a zero-node run must not pay for. The gate is enforced by the branch never reaching `createRunPlanner` — the cost is `prepareRsbuild` inside it, not `createNodeExecutor`, which allocates closures and nothing else (its dev server and worker pool are lazy, and its `init` is a no-op). Hoisting the planner call above the branch to unify the assembly is the regression this gate exists to prevent; folding the branch away means teaching the planner to skip the boot for a zero-node run, not moving the call.
