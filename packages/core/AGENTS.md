...
- Apart from the planner-owned shard banner above, the engine's list collection never writes to process streams; `rstest list` output is rendered only in `src/cli` from `ListCommandCollectionResult`.
