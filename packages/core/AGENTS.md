...
- A blob stores collected coverage before report-stage filtering or untested-file backfill. A run with the blob reporter must defer coverage reports and thresholds to `merge-reports`, which owns finalization against the unified map.
