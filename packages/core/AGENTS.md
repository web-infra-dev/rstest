...
- An empty browser watch launch keeps the CLI alive through its session and prints the ready banner so added test files can run, without raising the exit code; browser globalSetup runs even with zero entries because the session will run files added later, while non-watch keeps the entry gate. A boot failure returned as a cycle outcome can still open no session, in which case no ready banner prints and the CLI's config-restart watcher keeps the process alive.
