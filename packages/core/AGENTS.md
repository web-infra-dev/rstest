...
- `src/api/` — the public programmatic surface (`@rstest/core/api`). Today it sits _on top of_ `src/cli/`: it reuses that layer's config merge and runner builder, so the CLI and the API are siblings over a shared builder rather than a stack. Intended direction is the inverse — `runCLI` mapping argv onto `createRstest` — so prefer changes that move shared logic out of `src/cli/`, not further in.
...
- The exit code is a value: every layer that decides a run failed raises it on `context.exitCode`, which owns the never-downgrade rule (a later zero must not clear a prior non-zero within a cycle). `src/cli/` is the only writer of `process.exitCode` — including on behalf of `@rstest/browser` and of a coverage provider's host-process calls, which report through `ReportCoverageFailure`. It subscribes rather than reading once, because CLI-owned exit paths fire before the command promise settles. The code is cycle-scoped and reset with the snapshot summary in `prepareWatchCycleState`, so a programmatic host reads exactly the run it asked for.
- The programmatic API contains no save/restore of host state, and must not grow one: a guard would mean some layer writes a process global, and the fix belongs in that layer. What `!context.embedded` gates is not covered by this: `src/core` keeps the CLI-only code that owns the process (signal handlers, `process.exit`) _and_ the interactive control plane (stdin ownership, watch keypress shortcuts) — CLI-owned behavior living in `src/core` rather than `src/cli`. Adding another `!embedded` branch widens that debt — prefer injecting the behavior from the CLI over adding a site.
- `stateManager` reset is core-owned (top of a non-watch run, or `prepareWatchCycleState` ahead of every watch cycle, a session's first included) — executors never reset it, so bail reads stay cycle-scoped even where two executors' first cycles bracket one startup. The snapshot summary and the exit code are the halves a first cycle keeps, because the update-snapshot shortcut reads whatever the last cycle produced and an embedded host reads the startup's verdict — the browser's first cycle would otherwise clear what the node's just left.
- `globalSetup` teardown callbacks are registered against the `RstestContext` that ran the setup and drained by `runGlobalTeardown(context)`. One host process can hold several live contexts (concurrent programmatic runs, two reusable runners), so nothing may reintroduce a process-wide queue. A failing teardown raises the scope's exit code; the boolean it also returns exists only for `testRunner.close()`, which has no cycle result to attach the failure to and must reject instead.
- `globalSetup` env changes are a value too, never a host mutation: the worker's change-set is merged into `context.workerEnv` (`undefined` marks a key the setup deleted), and `composeWorkerEnv` is the single transport that puts it into any child of that run. Every new place that spawns or projects an environment must go through it; writing the host `process.env` to reach a worker would let concurrent contexts clobber each other's change-sets, and is not allowed. `initRstestEnv` is the one deliberate host write, and is limited to the two constant test-mode markers so concurrent runs cannot disagree — a value that varies per run must never join it.
...
- Browser projects' `globalSetup` runs once per watch session, before the initial browser cycle and before the node dev server starts: the stage mutates the host `process.env` and the pool re-reads it at dispatch, so a node cycle dispatched first would miss it. The node dependency check runs ahead of the stage in turn, so a node project that cannot run rejects the session before a user setup has mutated anything — that ordering is what `validateRunDependencies` exists as a seam member for. Reruns never re-run the stage (the host keeps the change-set for the session), and its teardown drains with the watch teardown; the en/zh `global-setup` docs state all three.
...
- One planner (`src/core/planner.ts`) is the run's init barrier: both sides' `modifyRstestConfig` hooks fire inside `createRunPlanner` (the browser's via a files-only discovery boot), so no executor is constructed against a plan still moving. The browser-side `src/core/browser/runPlanner.ts` is constructed and spent inside it — never hold a second planner to keep in step.
...
- A watch launch that opened no session prints no ready banner — no trigger could ever answer it. The no-test-files case is the host's sole direct `process.exitCode` write; a boot failure rides the cycle outcome. Either way the process stays alive on the CLI's config-restart watcher.
