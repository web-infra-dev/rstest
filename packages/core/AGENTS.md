...
- Compile callbacks deliver initial rejection through the driver's first-cycle Promise rather than throw into the compiler's fire-and-forget failure hook. Fatal bundler outcomes finalize and close the session through the driver; ordinary test compile errors keep watching. Mixed watch awaits browser startup rather than booting it in the background.
