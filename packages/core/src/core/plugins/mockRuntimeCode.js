              ...
              if ((key === 'readFileVm' || key === 'require') && obj[key]) {
                try {
                  if (
                    ...
                    console.warn(
                      '[Rstest Federation] Ignoring attempt to overwrite __webpack_require__.f.' +
                        String(key) +
...
if (globalThis.__rstest_federation__) {
  ...
  if (typeof __webpack_require__.f === 'object' && __webpack_require__.f) {
    for (const k of ['consumes', 'remotes']) {
      ...
      // Placeholder functions are generated by the bundler when it expects a
      // runtime plugin to supply the handler later. Those placeholders throw on
      // call, which breaks our eager chunk loading during test bootstrap.
      if (
        src.includes('should have __webpack_require__.f.') ||
        src.includes('should have __webpack_require__.f[')
      ) {
