...
__webpack_require__ = function (...args) {
  try {
    ...
    const isNotResolved =
      exports instanceof Promise &&
      utils.inspect(exports).startsWith('Promise {\n  <pending>,');

    if (isNotResolved && cached && cached.exports) {
      // TODO: support circular dependency ;)
      // Currently, without the error throwing below, circular dependency will raise dead lock of interdependent promises.
      // To resolve the issue, we need:
      // 1. get the exports inside the Promise and return it directly
      // 2. change all named import to namespace import and evaluate the real value in runtime instead of when init the module.
      // const originalExports = cached?._rstest_exports?.(); // _rstest_exports and escape for real export inside promise
      // if (originalExports) {
      //   return originalExports;
      // }

      throw new Error(
        `[Rstest] Circular dependency of module "${moduleId}" is detected, Rstest cannot resolve it, please consider resolve the circular dependency.`,
      );
    }
    const result = originalRequire(...args);
    return result;
  } catch (e) {
...
__webpack_require__.rstest_mock = (id, modFactory) => {
  ...
  if (typeof modFactory === 'string' || typeof modFactory === 'number') {
    ...
    const isAsync = modFactory.constructor.name === 'AsyncFunction';
    if (isAsync) {
