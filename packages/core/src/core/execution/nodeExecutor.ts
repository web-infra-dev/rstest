...
export function createNodeExecutor(
  ...
  let notifyCompileFailed:
    ((hint: { isFirstCompile: boolean }) => void) | undefined;
  ...
  const runCycle = async (
    ...
    const projectPlans = await Promise.all(
      ...
      if (error instanceof CompileFailedError) return error;
      throw error;
    });
    if (projectPlans instanceof CompileFailedError) {
      return {
        results: [],
        testResults: [],
    ...
    return {
      ...
      errors: [
        ...returns.flatMap((r) => ('errors' in r ? r.errors : [])),
        ...workerCleanupErrors,
      ],
      failure: returns.some((r) => 'failure' in r && r.failure === 'setup')
        ? 'setup'
        : undefined,
  ...
  const onInvalidate = (cb: ExecutorInvalidationCallback): void => {
    const onCompileEnd = async (isFirstCompile: boolean) => {
      ...
      try {
        ...
        // The driver delivers initial rejection through its first-cycle Promise.
        // Compiler callbacks must not also turn it into a failed compilation.
    ...
    notifyCompileFailed = ({ isFirstCompile }) => {
      // Failed compiles have no baseline to consume, so this hook need not be held.
      void onCompileEnd(isFirstCompile);
    };
  };
