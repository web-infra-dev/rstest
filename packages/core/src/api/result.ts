 ...
 * no-tests / `passWithNoTests` derivation and for that rerun's exit code. A
 * throwing `onResult` is isolated so a caller callback can't tear down the
 * watch session.
 *
 * The summary arrives at `onTestRunEnd` but is held until the cycle ends:
 * coverage thresholds raise after the reporters were notified, so emitting
 * immediately would report `ok: true` for a run the CLI fails.
 */
export const createResultReporter = (
  ...
  let pending:
    { files: TestFileResult[]; captured: CapturedRunState } | undefined;

  context.exitCode.onCycleEnd(() => {
    const settled = pending;
    // Release before the callback runs: a watch session idles between reruns,
    // and `captured` holds that run's whole coverage map.
    pending = undefined;
    if (!settled) {
      return;
    }
    try {
      onResult(assembleTestRunResult(settled.files, settled.captured, context));
    } catch {
  ...
  return {
    onTestRunEnd: (args) => {
      ...
      fillCapturedFromRunEnd(captured, args);
      pending = { files: args.results.map(toPublicTestFileResult), captured };
    },
  };
};
...
export const assembleTestRunResult = (
  ...
  // `passWithNoTests` is set. Derived here rather than read off the run's exit code
  // because a run that never reached finalize (a config load error) has no
  // code to read. Watch is exempt — the CLI never fails a watch rerun for
  // zero test files (a benign no-op rerun logs "No test files need re-run"
  // without an exit code), so a watch `onResult` must not report `ok: false`
  // for it either.
  ...
  const ok =
    ...
    !noTestsFailure &&
    // Failures that produce no failing test (coverage thresholds/report
    // generation, global teardown) never touch the stats above, so fold in the
    // run's exit code to mirror the CLI's pass/fail.
