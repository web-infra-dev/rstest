...
export async function createRstestInstance(
  ...
  if ('content' in config && 'filePath' in config) {
    result = {
      ...config,
      dependencies: (config.dependencies ?? []).map((file) =>
        getAbsolutePath(cwd, file),
      ),
  ...
  const build = async (
    ...
    const engine = await buildResolvedRunner({
 ...
 */
export const createRstest = (
  options: CreateRstestOptions = {},
): Promise<RstestInstance> => createRstestInstance(options, { embedded: true });
