...
- Worker env is composed, never inherited: both the spawn env and the per-dispatch `RuntimeConfig.env` come from `composeWorkerEnv(context.workerEnv)`, so `NODE_ENV`/`RSTEST`, `getForceColorEnv()` and this run's `globalSetup` change-set are explicit values. The pool is always created _before_ `globalSetup` runs, so its spawn env is a pre-setup snapshot and the change-set reaches workers through the dispatch-time projection — don't spread `process.env` here again, and don't move the projection to pool-creation time.
