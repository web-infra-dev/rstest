...
- Assets have two delivery paths that must both stay alive: eager on the task for forks/threads when host memory suffices, else lazily pulled by the worker via `rpc.getAssetsByEntry`. `vmThreads` always uses the lazy path so its worker can retain immutable asset bytes across fresh VM Contexts; the request carries only missing asset names after the first task, and `buildId`/worker disposal invalidate that cache. Both paths pass Rspack output bytes through `prepareAssetFilesForIPC`; worker consumers must use `getAssetText` or `getAssetBuffer` rather than assuming a transport-specific value shape.
