...
- Report-stage failures are caught and downgraded to an `exitCode.raise(1)`, but the raw-resolution seam inside `finalizeRunCycle` rethrows — a resource-load rejection propagates out of finalize instead of downgrading.
- A provider swallows per-payload/per-file failures to still produce a partial report, so it reports them through the `onFailure` callback (`ReportCoverageFailure`) core passes into every host-process call (`resolveRawCoverage`, `generateCoverageForUntestedFiles`); core turns each into an `exitCode.raise(1)`. Providers must not write `process.exitCode` from those calls — they run inside the embedding host. `collect`/`collectRaw` are the exception: they only run in a test worker, so their exit-code write stays.
