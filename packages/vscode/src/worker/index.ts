...
export class Worker {
  ...
  /**
   * Takes the instance identity — `init` already stripped the per-invocation
   * `fileFilters`/`command`, which `run()`/`listTests()` apply instead. Adding a
   * third per-invocation field therefore only has to be stripped there, and it
   * can never leak into the inline config from here.
   */
  ...
  >): Promise<RstestInstance> {
    ...
    // where `loadConfig` lives (the programmatic API does not read a config
