...
export class Worker {
  private async init({
    ...
    const initializedOptions = await initCli({ config: configFilePath });
    logger.debug('initializedOptions', initializedOptions);

    const rstest = createRstest(
      {
        config: {
          ...initializedOptions.config,
          ...config,
          reporters: [
          ...
          coverage: {
            ...initializedOptions.config.coverage,
            ...config.coverage,
          },
        },
        configFilePath,
        projects: initializedOptions.projects,
      },
      command,
      fileFilters ?? [],
    );
