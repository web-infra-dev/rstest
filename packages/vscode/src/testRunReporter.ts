...
export class TestRunReporter implements Reporter {
  constructor(
    ...
    private testCaseNamePath: string[] = [],
  ) {}
  ...
  public async onOutput(message: string) {
  ...
  private findTestItem(value: TestCaseInfo | TestSuiteInfo | TestResult) {
    return this.generatePath(value).reduce<vscode.TestItem | undefined>(
      ...
      this.project?.testFiles.get(vscode.Uri.file(value.testPath).toString())
        ?.testItem,
    );
  }
  ...
  private contains(value: TestCaseInfo | TestSuiteInfo | TestResult) {
    ...
    if (path.length < this.testCaseNamePath.length) return false;
    return this.testCaseNamePath.every((name, index) => path[index] === name);
  }
  ...
  onTestFileStart(test: TestFileInfo) {
    ...
    if (!this.testCaseNamePath.length && fileTestItem)
      this.run?.started(fileTestItem);
  }
  onTestFileResult(test: TestFileResult) {
    // only update test file result when explicit run it
    if (this.testCaseNamePath.length) return;
    const fileTestItem = this.project?.testFiles.get(
      vscode.Uri.file(test.testPath).toString(),
    )?.testItem;
    ...
    switch (test.status) {
      case 'todo':
      case 'skip':
        this.run?.skipped(fileTestItem);
        break;
      case 'pass':
        this.run?.passed(fileTestItem, test.duration);
        break;
      case 'fail':
        this.run?.failed(fileTestItem, [], test.duration);
        break;
    }
  }
