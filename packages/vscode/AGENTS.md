...
Two-process design: the extension (`src/extension.ts`, TestController + test tree) spawns a worker (`src/worker/`) that runs tests and reports back over WebSocket. The worker protocol types in `src/types.ts` are shared by both sides — a protocol change must land on both ends in the same commit.
