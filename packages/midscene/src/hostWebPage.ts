...
export class HostWebPage {
  ...
  async screenshotBase64(): Promise<string> {
    // Use 'device' scale so the screenshot is always at device-pixel resolution.
    // Playwright ≥1.59 fixed `scale: 'css'` to actually return CSS-pixel images,
    // which broke the DPR-based coordinate math below that assumed device pixels.
    // Using 'device' gives consistent behavior across Playwright versions.
