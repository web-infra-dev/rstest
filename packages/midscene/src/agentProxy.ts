...
const MIDSCENE_LOG_PREVIEW_MAX_LENGTH = 80;

const truncateLogPreview = (value: string): string => {
  if (value.length <= MIDSCENE_LOG_PREVIEW_MAX_LENGTH) {
    return value;
  }

  return `${value.slice(0, MIDSCENE_LOG_PREVIEW_MAX_LENGTH - 3)}...`;
};
...
const getLogPreviewFromArg = (arg: unknown): string | undefined => {
...
const logAiRpcRequest = (
  method: AiRpcMethod,
  args: readonly unknown[],
): void => {
  const preview = args.map(getLogPreviewFromArg).find(Boolean);
  const previewSuffix = preview ? ` - ${truncateLogPreview(preview)}` : '';

  console.log(`[midscene] 🤖 Executing AI action: ${method}${previewSuffix}`);
};
...
export class AgentProxy {
  ...
  private async sendVoid<M extends VoidAiRpcMethod>(
    ...
    logAiRpcRequest(method, args);
    await sendAiRpcRequest(method, args);
  }
