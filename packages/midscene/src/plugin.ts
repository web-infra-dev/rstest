...
/**
 * Internal Rsbuild plugin used by `withMidscene()`.
 */
  ...
  return {
    ...
    setup(api) {
      api.modifyRspackConfig((config) => {
        ...
        if (!ignored.includes('**/midscene_run/**')) {
          ignored.push('**/midscene_run/**');
        }

        config.watchOptions.ignored = ignored as NonNullable<
          NonNullable<typeof config.watchOptions>['ignored']
        >;
      });
      ...
      api.onAfterStartDevServer(async () => {
        ...
        const providerContext = browserApi.provider;
        ...
        if (existsSync(envPath)) {
          try {
            ...
            logMidsceneWarning(
              `Failed to load Midscene env file at ${envPath}: ${
