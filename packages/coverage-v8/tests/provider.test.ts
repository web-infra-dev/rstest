...
import {
  __clearCoverageAstCachesForTest,
...
describe('coverage-v8 provider', () => {
  ...
  it('deduplicates prepared coverage by original source', async () => {
    const root = join(tmpdir(), 'rstest-coverage-v8-original-source-cache');
    const sourceFile = join(root, 'src', 'original.ts');
    const generatedFileA = join(root, 'dist', 'a.js');
    const generatedFileB = join(root, 'dist', 'b.js');
    ...
    const createCoverage = async (generatedFile: string, source: string) =>
      convertV8CoverageWithAst({
        ast: () => parseModule(code),
        cacheKey: `${generatedFile}:original-source-cache`,
        code,
        coverage: {
          url: pathToFileURL(generatedFile).href,
          functions: [
        ...
        sourceMap: createSourceMap(source),
        sourceMapUrl: `${generatedFile}.map`,
      });

    try {
      const firstCoverage = await createCoverage(
        generatedFileA,
        '../src/original.ts',
      );
      const secondCoverage = await createCoverage(generatedFileB, sourceFile);

      expect(parseCalls).toBe(1);
      expect(Object.keys(firstCoverage)).toEqual([sourceFile]);
      expect(Object.keys(secondCoverage)).toEqual([sourceFile]);
    } finally {
      Parser.parse = originalParse;
    }
  });

  it('keeps small remapped single-source files on the generated-code path', async () => {
    const root = join(tmpdir(), 'rstest-coverage-v8-small-original-source');
    const generatedFile = join(root, 'dist', 'bundle.js');
    const originalFile = join(root, 'src', 'original.ts');
    const generatedCode = 'const value = 1;\nvalue;';
    const originalCode = 'export const value = 1;\nvalue;';
    const ast = parseModule(generatedCode);
    const originalParse = Parser.parse.bind(Parser);
    let parseCalls = 0;
    ...
    try {
      const coverage = await convertV8CoverageWithAst({
        ast,
        cacheKey: `${generatedFile}:small-original-source`,
        code: generatedCode,
        coverage: {
          url: pathToFileURL(generatedFile).href,
          functions: [
  ...
  it('falls back to generated code conversion when original source is not parseable JavaScript', async () => {
    const root = join(
      tmpdir(),
      'rstest-coverage-v8-original-source-ts-fallback',
    );
    const generatedFile = join(root, 'dist', 'bundle.js');
    const originalFile = join(root, 'src', 'original.ts');
    const code = 'const value = 1;';
    ...
    const coverage = await convertV8CoverageWithAst({
      ast,
      cacheKey: `${generatedFile}:original-source-ts-fallback`,
      code,
      coverage: {
        url: pathToFileURL(generatedFile).href,
        functions: [
...
`,
  ...
  it('filters raw asset coverage when source maps only point to excluded sources', async () => {
    const root = join(tmpdir(), 'rstest-coverage-v8-raw-map-filter');
    const generatedFile = join(root, 'dist', 'bundle.js');
    const excludedOriginalFile = join(root, 'test', 'helper.ts');
    const sourceMap = {
      version: 3,
      sources: ['../test/helper.ts'],
      names: [],
      mappings: 'AAAA',
    };
    const code = `value();\n//# sourceMappingURL=data:application/json;charset=UTF-8,${encodeURIComponent(
    ...
    Object.defineProperty(providerInternals, 'convertWithAst', {
      ...
      value: async (filePath: string) => {
        convertedFiles.push(filePath);
        return {
          [excludedOriginalFile]: createFileCoverage(excludedOriginalFile),
        };
      },
    });
    ...
    expect(convertedFiles).toEqual([]);
    expect(coverageMap?.files()).toEqual([]);
  });

  it('keeps raw asset coverage when source maps point to included sources', async () => {
    const root = join(tmpdir(), 'rstest-coverage-v8-raw-map-keep');
    const generatedFile = join(root, 'dist', 'bundle.js');
    const originalFile = join(root, 'src', 'original.ts');
    const sourceMap = {
      version: 3,
    ...
    Object.defineProperty(providerInternals, 'convertWithAst', {
      ...
      value: async (filePath: string) => {
        convertedFiles.push(filePath);
        return { [originalFile]: createFileCoverage(originalFile) };
      },
    });
    ...
    expect(convertedFiles).toEqual([generatedFile]);
    expect(coverageMap?.files()).toEqual([originalFile]);
  });

  it('filters raw coverage when provided source maps only point to excluded sources', async () => {
    const root = join(tmpdir(), 'rstest-coverage-v8-raw-provided-map-filter');
    const generatedFile = join(root, 'dist', 'bundle.js');
    const excludedOriginalFile = join(root, 'test', 'helper.ts');
    const provider = new CoverageProvider(
      createOptions({ include: ['src/**/*.ts'] }),
    ...
    const coverageMap = await provider.collect({
      assetFiles: { [generatedFile]: 'value();' },
      sourceMaps: {
