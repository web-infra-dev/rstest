...
import type { BrowserRpcResponse } from '../src/rpcProtocol';
...
describe('browserRpc client', () => {
  ...
  const respond = (response: BrowserRpcResponse) => {
    if (!messageHandler) {
      throw new Error('message handler is not initialized');
    }
    messageHandler({
      data: {
        type: '__rstest_browser_rpc_response__',
        payload: response,
      },
    } as MessageEvent);
  };
  ...
  it('should include runId and uuid request id', async () => {
    ...
    const postedPayload = mockPostMessage.mock.calls[0]?.[0] as {
      payload?: {
        payload?: { id?: string; testPath?: string; runId?: string };
      };
    };
    const request = postedPayload.payload?.payload;

    expect(request?.id).toBe('rpc-id-1');
    expect(request?.testPath).toBe('/tests/example.test.ts');
    expect(request?.runId).toBe('run-1');

    respond({ id: request!.id!, result: undefined });
