...
- A headed cycle ends only once every reload it issued has settled, and each settlement normally arrives as an event from the container. Any host action that makes such an event impossible — dropping a file from the set, and with it the iframe its runner was navigating — must settle that reload itself, in the same step, before the container is told. An unsettled reload wedges its cycle and every cycle core has queued behind it, with no error and no disconnect to show for it, so a new way to make a runner unreachable is a new settlement obligation, not just a new log line.
