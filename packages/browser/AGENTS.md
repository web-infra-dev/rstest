...
- Watch startup must wait for the project compilers to finish their additional passes and reattach filesystem watchers before exposing a ready session. An empty first cycle has no runner page to wait for assets implicitly; signalling ready from a compile-done hook lets the host discover new files that the emitted context does not contain.
