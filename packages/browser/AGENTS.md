...
- Core's `finalizeRunCycle` owns reporters `onTestRunEnd`, coverage merge, and the exit code for every cycle on both commands. The host never finalizes: test failures and fatal compile errors reach core as cycle outcomes, while config, dev-server, and browser-launch failures reject startup.
