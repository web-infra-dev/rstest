...
- Core's `finalizeRunCycle` owns reporters `onTestRunEnd`, coverage merge, and the exit code for every cycle on both commands. The host never finalizes: test failures and fatal compile errors reach core as cycle outcomes, while config, dev-server, and browser-launch failures throw out of `runCycle`. Core converts these rejections into a `'setup'` outcome: `run()` resolves a failed result, and the first watch cycle ends the session after finalize.
...
- Watch startup must wait for the project compilers to finish their additional passes and reattach filesystem watchers before exposing a ready session. If any project rebuilds while another is pending, readiness must be rechecked across all projects. An empty first cycle has no runner page to wait for assets implicitly; signalling ready from a compile-done hook lets the host discover new files that the emitted context does not contain.
...
- Rerun-cycle executor rejections finalize an error cycle without ending the watch session. Mixed watch awaits browser startup rather than booting it in the background. Embedded hosts get no CLI shortcut hints, even on a TTY.
...
- Host-side Playwright launches pin `handleSIGINT`, `handleSIGTERM`, and `handleSIGHUP` to `false` after user options, while the dev server runs in `middlewareMode`, so neither dependency installs process signal listeners in the host.
