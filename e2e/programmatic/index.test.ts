...
type OnTestFinished = Parameters<typeof runRstestCli>[0]['onTestFinished'];
...
/**
 * Spawn one `fixtures/*.mjs` bridge and parse the payload it prints. Every
 * programmatic fixture is driven the same way, so the spawn shape lives here
...
const runApiFixture = async (
  ...
  return { exec, result: parsePayload(cli.stdout) };
};
...
describe('programmatic createRstest', () => {
  it('runs disk tests via inline config object + returns nested stats', async ({
    ...
    const { result } = await runApiFixture('run-inline.mjs', onTestFinished);
  ...
  it('eager context build preserves the resolved reporter config', async ({
    ...
    const { result } = await runApiFixture(
      'run-create-context-reporters.mjs',
      onTestFinished,
    );
  ...
  it('reports failures via ok=false without poisoning host process.exitCode', async ({
    ...
    const { exec, result } = await runApiFixture(
      'run-failing.mjs',
      onTestFinished,
    );
  ...
  it('config factory loads the disk config itself and transforms it', async ({
    ...
    const { result } = await runApiFixture('run-config-fn.mjs', onTestFinished);
  ...
  it('never mutates host process globals, during the run or after it', async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-host-globals.mjs',
      onTestFinished,
    );

    // The run really did fail — a failure that must be published through
    // `result.ok`, never through a process global.
    expect(result.ok).toBe(false);
    expect(result.failedTests).toBe(1);
    // A concurrent timer sampled the host throughout the run, so this is
    // "never written", not "written and put back" — a distinction only a
    // mid-run probe can make.
    expect(result.sampleCount).toBeGreaterThan(5);
    expect(result.samplesWithExitCode).toEqual([]);
    // The host env is likewise stable mid-run: the API claims the CLI's two
    // test-mode markers up front and writes nothing else, ever.
    expect(result.samplesWithUnexpectedEnv).toEqual([]);
    // Positive control: the probe really was reading fields that started empty.
    expect(result.before).toEqual({
      exitCode: null,
      NODE_ENV: null,
      RSTEST: null,
    });
    expect(result.afterCreate).toEqual({
      exitCode: null,
      NODE_ENV: 'test',
      RSTEST: 'true',
    });
    expect(result.afterRun).toEqual(result.afterCreate);
  });

  it('keeps a globalSetup env change inside the run, never on the host', async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-global-setup-env.mjs',
      onTestFinished,
    );

    // The worker test asserts it saw the globalSetup value, so a passing run is
    // the proof the change-set reached the pool.
    expect(result.ok).toBe(true);
    expect(result.passedTests).toBe(1);
    // ...and it got there without the host ever holding the variable.
    expect(result.before).toBe(null);
    expect(result.after).toBe(null);
    expect(result.sampleCount).toBeGreaterThan(5);
    expect(result.dirtyDuring).toEqual([]);
  });

  it('isolates concurrent runs whose globalSetups set the same env key', async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-concurrent-global-setup-env.mjs',
      onTestFinished,
    );

    // Each run's workers read their own run's value. Routed through the shared
    // host env, the second setup would overwrite the first and one of these
    // would fail — this is the guarantee removing that transport buys.
    expect(result.alpha).toEqual({ ok: true, passed: 1, failed: 0 });
    expect(result.beta).toEqual({ ok: true, passed: 1, failed: 0 });
    expect(result.after).toBe(null);
    expect(result.dirtyDuring).toEqual([]);
    expect(result.hostExitCode).toBe(0);
  });

  it('leaves host process.env alone when creation fails', async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-create-failure.mjs',
      onTestFinished,
    );

    expect(result.threw).toBe(true);
    expect(result.before).toEqual({ NODE_ENV: null, RSTEST: null });
    // A creation failure leaves the same two markers a successful one does —
    // no half-applied env, and nothing to unwind.
    expect(result.after).toEqual({ NODE_ENV: 'test', RSTEST: 'true' });
  });

  it('listTests({ printLocation: true }) collects task locations', async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-list-location.mjs',
      onTestFinished,
    );
  ...
  it('tracks the loaded config file for buildCache dependency resolution', async ({
    ...
    const { result } = await runApiFixture(
      'run-config-file-buildcache.mjs',
      onTestFinished,
    );
  ...
  it('watch() runs, exposes a closeable watcher, and tears down cleanly', async ({
    ...
    // the fixture's exec would never resolve.
    const { exec, result } = await runApiFixture(
      'run-watch.mjs',
      onTestFinished,
    );
  ...
  it('fails the run (ok=false) when a coverage threshold is not met, even though every test passes', async ({
    ...
    expect(result.ok).toBe(false);
    // The engine reported it as a value, so the embedder isn't poisoned.
    expect(result.hostExitCode).toBe(0);
  });

  it('fails the CLI with a non-zero exit code for that same coverage threshold', async ({
    onTestFinished,
  }) => {
    // Same fixture, same config file (the runner script above imports the very
    // config this CLI run discovers), other surface: the CLI is now the only
    // layer that turns the engine's exit code into a process exit code, so the
    // two surfaces have to keep agreeing.
    const { expectExecFailed, expectStderrLog } = await runRstestCli({
      command: 'rstest',
      args: ['run'],
      onTestFinished,
      options: {
        nodeOptions: { cwd: join(fixturesDir, 'coverage-threshold') },
      },
    });

    await expectExecFailed();
    // Pin the *reason* for the non-zero exit: without this, an unrelated
    // startup failure would satisfy `expectExecFailed` just as well.
    expectStderrLog(/does not meet global threshold/i);
  });

  it('fails the run (ok=false) when the coverage provider cannot instrument an untested file', async ({
    onTestFinished,
  }) => {
    const generatedDir = join(fixturesDir, 'test-temp-coverage-untested');
    onTestFinished(() =>
      rmSync(generatedDir, { recursive: true, force: true }),
    );

    const { result } = await runApiFixture(
      'run-coverage-untested-failure.mjs',
      onTestFinished,
    );

    // The provider recovers from the failure (it still emits a partial report),
    // so this only surfaces if it reports the failure back to core as a value.
    expect(result.stats.tests.failed).toBe(0);
    expect(result.ok).toBe(false);
    // The provider runs in the host process here (untested-file backfill), so
    // this is the case that would silently poison a long-lived embedder.
    expect(result.hostExitCode).toBe(0);
  });
  ...
  it('accepts inline config + virtual modules plugin (Midscene shape)', async ({
    ...
    const { result } = await runApiFixture('run-virtual.mjs', onTestFinished);

    expect(result.ok).toBe(true);
    expect(result.stats.tests.passed).toBe(1);
    expect(result.files).toEqual([
      { status: 'pass', testPath: 'virtual/programmatic.test.ts' },
    ]);
    expect(result.hostExitCode).toBe(0);
  });
  ...
  it('returns metadata from test context and suite hooks', async ({
    ...
    const { result } = await runApiFixture('run-metadata.mjs', onTestFinished);
  ...
  it('run({ related }) runs only related test files for a source file (jest findRelatedTests parity)', async ({
    ...
    const { result } = await runApiFixture('run-related.mjs', onTestFinished);
...
describe('programmatic createRunner (build once, run many)', () => {
  ...
  it("does not leak a failing run's exit code into the next run on the same runner", async ({
    onTestFinished,
  }) => {
    const { result } = await runApiFixture(
      'run-runner-exit-code-reset.mjs',
      onTestFinished,
    );

    expect(result.failing).toEqual({ ok: false, passed: 0, failed: 1 });
    // One runner means one context and one exit-code carrier — without a
    // per-cycle reset this passing run would inherit the failure above.
    expect(result.passing).toEqual({ ok: true, passed: 2, failed: 0 });
    // The reset clears the code rather than disabling the carrier.
    expect(result.failingAgain).toEqual({ ok: false, passed: 0, failed: 1 });
    expect(result.hostExitCode).toBe(0);
  });

  it('close() is idempotent and leaves the host env + exit code as it found them', async ({
    onTestFinished,
  }) => {
    const { exec, result } = await runApiFixture(
      'run-runner-close.mjs',
      onTestFinished,
    );
    ...
    expect(result.failedTests).toBe(1);
    expect(result.before).toEqual({ NODE_ENV: null, RSTEST: null });
    // A live runner holds no host state hostage: the failing run's exit code
    // never reached the host's exit code, which is still the 3 the host set for
    // its own reasons, and the env is just the two test-mode markers.
    expect(result.duringRunner).toEqual({
      env: { NODE_ENV: 'test', RSTEST: 'true' },
      exitCode: 3,
    });
    // close() releases resources only; there is no host state to put back.
    expect(result.after).toEqual({ NODE_ENV: 'test', RSTEST: 'true' });
